...
async function migrateInventoryData() {
  ...
  try {
    ...
    professionalLogger.debug("📖 Scanning for players with obsolete inventory formats...", {
      correlationId,
    });
    const usersSnapshot = await db.collection("players").where("inventory", "!=", null).get();

    professionalLogger.info(`👥 Found ${usersSnapshot.size} potential users to audit`, {
      correlationId,
    });
...
async function processSingleUser(
  ...
  professionalLogger.info(`📦 Migrating user ${userId} (${oldInventory.length} items)`, {
