...
async function main() {
    ...
    try {
        const baseline = await runScenario({ title: 'Baseline Traffic', connections: 50, duration: 10 });
        const stress = await runScenario({ title: 'Stress Test (Attack Flood)', connections: 500, duration: 10, pipelining: 4 });
