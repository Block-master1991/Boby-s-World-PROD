...
export async function runSessionTests(runner: SecurityTestSuite, getMockDevice: () => DeviceInfo) {
    ...
    await runner.runTest('SessionManager - Validate Session', async () => {
        ...
        if (!session) throw new Error('Failed to create session');
        const validation = await sessionManager.validateSession(session.sessionId, testDeviceInfo, '127.0.0.1');
...
export async function runRateLimiterTests(runner: SecurityTestSuite, getMockDevice: () => DeviceInfo) {
    ...
    await runner.runTest('RateLimiter - Normal Request', async () => {
        ...
        const result = await rateLimiter.checkRateLimit(mockRequest as unknown as Request, 'test-user', { endpoint: '/api/test', deviceInfo });
...
export async function runSecurityIntegrationTests(runner: SecurityTestSuite) {
    ...
    await runner.runTest('SecurityIntegration - Comprehensive Statistics', async () => {
        ...
        if (!stats.keyVault || !stats.sessions || !stats.rateLimiting) throw new Error('Incomplete statistics');
        return stats;
    });
}

export async function runPerformanceTests(runner: SecurityTestSuite, getMockDevice: () => DeviceInfo) {
    await runner.runTest('Performance - Key Creation', async () => {
    ...
    await runner.runTest('Performance - Session Management', async () => {
        const startTime = Date.now();
        const deviceInfo = getMockDevice();
...
async function runRepeatedRequestsTest(runner: SecurityTestSuite) {
    await runner.runTest('Security - Protection Against Repeated Requests', async () => {
        ...
        for (let i = 0; i < 50; i++) {
            // eslint-disable-next-line no-await-in-loop
            const result = await rateLimiter.checkRateLimit(mockRequest as unknown as Request, 'attack-user', { endpoint: '/api/attack' });
...
async function runEncryptionTest(runner: SecurityTestSuite) {
    await runner.runTest('Security - Encryption Integrity', async () => {
        ...
        if (decrypted1 !== sensitiveData || decrypted2 !== sensitiveData) throw new Error('Failed to decrypt data correctly');
        return { encryptionConsistent: true, dataIntegrity: true, warning };
    });
}

async function runSessionProtectionTest(runner: SecurityTestSuite, getMockDevice: () => DeviceInfo) {
    await runner.runTest('Security - Session Protection', async () => {
        ...
        const validation1 = await sessionManager.validateSession(session.sessionId, deviceInfo1, '127.0.0.1');
        const validation2 = await sessionManager.validateSession(session.sessionId, deviceInfo2, '127.0.0.1');
...
async function runSessionRevocationTest(runner: SecurityTestSuite, getMockDevice: () => DeviceInfo) {
    await runner.runTest('Security - Session Revocation', async () => {
        ...
        const validation = await sessionManager.validateSession(session.sessionId, deviceInfo, '127.0.0.1');
