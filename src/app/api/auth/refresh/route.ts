...
async function rotateTokens(refreshToken: string, nonce: string, metadata: { userAgent: string; ip: string; host?: string }) {
  ...
  if (!payload || payload.nonce !== nonce) {
    ...
    return createAuthErrorResponse({ message: "Invalid session. Please login again.", code: "AUTH_MISMATCH", status: 403, clearCookies: true });
  }

  const result = await JWTManager.refreshAccessToken(refreshToken, userAgent, ip);
  if (!result) {
    return createAuthErrorResponse({ message: "Session expired. Please login again.", code: "REFRESH_EXPIRED", status: 401, clearCookies: true });
  }
  ...
  response.cookies.set("accessToken", result.accessToken, JWTManager.createSecureCookieOptions(15 * 60, host));
...
export const POST = withCsrfProtection(async (request: Request) => {
  ...
  try {
    ...
    if (!token || !nonce) {
      return createAuthErrorResponse({ message: "Missing tokens", code: "MISSING_AUTH_DATA", status: 401, clearCookies: true });
    }
    ...
    const response = createAuthErrorResponse({ message: "Token refresh failed", code: "REFRESH_ERROR", status: 500 });
