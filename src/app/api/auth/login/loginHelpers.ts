...
async function checkFirestoreConnectivity(
  ...
  try {
    ...
    logger.error(
      `[AuthNonces] Firestore connectivity check FAILED (${docName}):`,
...
export async function generateNonce(publicKey: string): Promise<string | null> {
  ...
  try {
    ...
    logger.log(
      `[AuthNonces] Nonce ${newNonce} stored for ${publicKey}. Expiry: ${new Date(expiry).toISOString()}`
    );
    return newNonce;
  } catch (error) {
    logger.error(
      `[AuthNonces] Error in generateNonce for ${publicKey}:`,
...
interface NonceVerifyResult {
  success: boolean;
  reason: string;
}

function executeNonceTransaction(
  db: FirebaseFirestore.Firestore,
  publicKey: string,
  clientNonce: string
  ...
  return db.runTransaction(async transaction => {
    ...
    const { nonce, expiry, attempts } = nonceDoc.data() as {
      nonce: string;
      expiry: number;
      attempts: number;
    };
...
export async function verifyAndConsumeNonce(
  ...
  try {
    await initializeAdminApp();
    const db = getFirestore();
    if (!(await checkFirestoreConnectivity(db, "connectivity_verify_nonce")))
    ...
    logger.log(
      `[AuthNonces] Transaction for ${publicKey}: Success=${result.success}, Reason=${result.reason}`
    );
    return result;
  } catch (error) {
    logger.error(
      `[AuthNonces] Transaction error for ${publicKey}:`,
      error instanceof Error ? error.message : String(error)
    );
...
export async function ensureFirestoreConnectivity(docId: string): Promise<NextResponse | null> {
  try {
    ...
    logger.error(
      `[Login] Firestore check FAILED (${docId}):`,
      diagError instanceof Error ? diagError.message : String(diagError)
    );
    return NextResponse.json(
      {
        error: "Server configuration error with database.",
        details: `Connectivity check failed (${docId}).`,
      },
      { status: 500 }
    );
  }
...
export async function createOrUpdatePlayerDoc(
  db: FirebaseFirestore.Firestore,
  publicKey: string
): Promise<void> {
  try {
    ...
    logger.error(
      "[Login] Error creating/updating player doc:",
