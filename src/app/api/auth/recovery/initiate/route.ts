...
export const POST = withCsrfProtection(async (request: Request) => {
    try {
        ...
        if (!eligibility.allowed) {
            return NextResponse.json({ error: eligibility.reason }, { status: eligibility.status || 400 });
        }

        // 2. User & Email Verification
        const { exists, hasPasskeys, emailMatch } = await RecoveryService.verifyUserRecord(publicKey, email);
        if (!exists) return NextResponse.json({ error: 'User not found' }, { status: 404 });
        if (!emailMatch) return NextResponse.json({ error: 'Email does not match the registered email for this wallet' }, { status: 403 });
        if (!hasPasskeys) return NextResponse.json({ error: 'No passkeys found for this account' }, { status: 404 });

        // 3. Initiate Recovery
        const recoveryToken = await RecoveryService.initiateRecovery(publicKey, email, metadata);
        if (!recoveryToken) return NextResponse.json({ error: 'Internal Server Error' }, { status: 500 });
        ...
        logger.error('[Recovery Initiate] Error:', error instanceof Error ? error.message : String(error));
