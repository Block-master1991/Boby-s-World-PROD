...
async function blacklistTokens(request: Request, accessToken: string | undefined, refreshToken: string | undefined): Promise<string | undefined> {
    ...
    if (accessToken) {
        ...
        try {
            ...
            const payload = await JWTManager.verifyAccessToken(accessToken, userAgent, getClientIp(request));
            if (payload?.sub) userPublicKey = payload.sub;
        } catch (tokenError) {
            logger.warn('[LOGOUT] Could not extract publicKey from accessToken:', tokenError instanceof Error ? tokenError.message : String(tokenError));
        }
    }
    if (refreshToken) {
...
async function terminateUserSessions(request: Request, userPublicKey: string): Promise<void> {
    ...
    await auditLogger.logEvent('LOGOUT', `User logged out: ${userPublicKey}`, { userId: userPublicKey, ip: getClientIp(request), endpoint: '/api/auth/logout' }, 'info');
}

function clearAuthCookies(response: NextResponse, requestHost: string | undefined): void {
    const commonExpiredOptions = { ...JWTManager.createSecureCookieOptions(-1, requestHost), expires: new Date(0) };
    const csrfExpiredOptions = { expires: new Date(0), path: '/' };

    const cookiesToClear = ['accessToken', 'refreshToken', 'session', 'nonce', 'secure_session', 'session_seed'];
...
export const POST = withCsrfProtection(async (request: Request) => {
    ...
    try {
        ...
        const response = NextResponse.json({ success: true, message: 'Logged out successfully. All session cookies cleared.' });
        clearAuthCookies(response, request.headers.get('host') || undefined);

        logger.log('[LOGOUT] Logout process completed');
        return response;
    } catch (error) {
        logger.error('[POST /api/auth/logout] Error:', error instanceof Error ? error.message : String(error));
        return NextResponse.json({ error: 'Logout failed', details: process.env['NODE_ENV'] === 'development' ? (error instanceof Error ? error.message : 'Unknown error') : 'Internal server error' }, { status: 500 });
    }
});
