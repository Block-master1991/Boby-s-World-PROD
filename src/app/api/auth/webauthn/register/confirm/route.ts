...
export const POST = withAuth(withCsrfProtection(async (request: AuthenticatedRequest) => {
    try {
        const { credential, description, transports } = await validateRequestBody(request, WebAuthnConfirmSchema);
        const userId = request.user.sub;

        if (!userId) return NextResponse.json({ error: 'Authenticated UserID required' }, { status: 401 });

        const key = WebAuthnService.getChallengeKey('registration', userId);
        const storedChallenge = await redis.get(key);
        if (!storedChallenge) return NextResponse.json({ error: 'Registration session expired' }, { status: 400 });

        const existingPasskey = await db.collection('players').doc(userId).collection('passkeys').doc(credential.id).get();
        if (existingPasskey.exists) return NextResponse.json({ error: 'Passkey already registered' }, { status: 409 });
        ...
        logger.error('[WebAuthn Confirm] Error:', error instanceof Error ? error.message : String(error));
