...
function attachProcessListeners(
    ...
    proc.stdout?.on('data', (data: Buffer) => { output += data.toString(); logger.log('Migration output:', data.toString()); });
    proc.stderr?.on('data', (data: Buffer) => { errorOutput += data.toString(); logger.error('Migration error:', data.toString()); });
    proc.on('close', async (code) => {
        logger.log(`Migration process exited with code ${code}`);
        if (code === 0) {
            const response = NextResponse.json({ success: true, message: 'Migration completed successfully', output, code });
            const requestHost = request.headers.get('host') || undefined;
            resolve(await setCsrfTokenResponse(response, request.user.sub, requestHost));
        } else {
            resolve(NextResponse.json({ success: false, error: 'Migration failed', output, errorOutput, code }, { status: 500 }));
        }
    });
    proc.on('error', (error: Error) => {
        logger.error('Failed to start migration process:', error.message);
        resolve(NextResponse.json({ success: false, error: 'Failed to start migration process', details: error.message }, { status: 500 }));
    });
}
...
function runMigrationProcess(request: AdminRequest): Promise<NextResponse> {
    ...
    return new Promise((resolve) => {
        const proc = spawn('npx', ['ts-node', '-r', 'dotenv/config', scriptPath], {
            stdio: ['inherit', 'pipe', 'pipe'], cwd: process.cwd(), env: getMigrationEnvVars()
        });
...
export const POST = withSignedAdminAuth(withCsrfProtection(async (request: AdminRequest) => {
    try {
        ...
        logger.error('Error in migration API:', error instanceof Error ? error.message : String(error));
...
async function checkMigrationStatus(): Promise<MigrationResult> {
    ...
    if (migrationDoc.exists) {
        return { success: true, message: 'Migration has run', output: JSON.stringify(migrationDoc.data()) };
    }
...
export const GET = withAdminAuth(async () => {
    try {
        ...
        logger.error('Error checking migration status:', error instanceof Error ? error.message : String(error));
        return NextResponse.json({ success: false, error: 'Failed to check migration status', details: error instanceof Error ? error.message : 'Unknown error' }, { status: 500 });
    }
});
