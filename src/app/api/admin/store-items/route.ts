...
export const GET = withAdminAuth(async () => {
    try {
        ...
        return NextResponse.json({ success: false, error: 'Failed to fetch store items', details: error instanceof Error ? error.message : 'Unknown error' }, { status: 500 });
    }
});

interface CreateItemBody { id?: string; name?: string; description?: string; price?: number; image?: string; dataAiHint?: string; type?: string; rarity?: string; }
...
async function processCreateItem(body: CreateItemBody, request: AdminRequest): Promise<NextResponse> {
    const input = {
        name: body.name!, description: body.description!,
        price: body.price || 0, image: body.image!, dataAiHint: body.dataAiHint || '',
    ...
    const response = NextResponse.json({ success: true, message: 'Item created successfully', item: newItem });
...
export const POST = withSignedAdminAuth(withCsrfProtection(async (request: AdminRequest) => {
    try {
        ...
        logger.error('Error creating store item:', error instanceof Error ? error.message : String(error));
        const isDuplicate = error instanceof Error && error.message.includes('already exists');
        return NextResponse.json({ success: false, error: isDuplicate ? 'An item with this ID already exists' : 'Failed to create store item', details: error instanceof Error ? error.message : 'Unknown error' }, { status: isDuplicate ? 409 : 500 });
    }
}));
