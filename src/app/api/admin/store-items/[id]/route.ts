...
import { deleteStoreItem, getStoreItemById, updateStoreItem } from "@/lib/server-items/server-items";
...
type UpdateBody = Partial<
  Pick<
    StoreItemDocument,
    "name" | "description" | "price" | "image" | "dataAiHint" | "type" | "rarity" | "isActive"
  >
>;
...
export const PUT = withSignedAdminAuth(
  withCsrfProtection(async (request: AdminRequest, { params }: RouteParams) => {
    try {
      ...
      return NextResponse.json(
        {
          success: false,
          error: isNotFound ? "Item not found" : "Failed to update store item",
          details: error instanceof Error ? error.message : "Unknown error",
        },
...
export const DELETE = withSignedAdminAuth(
  withCsrfProtection(async (request: AdminRequest, { params }: RouteParams) => {
    try {
      const resolvedParams = await params;
      const itemId = resolvedParams.id;
