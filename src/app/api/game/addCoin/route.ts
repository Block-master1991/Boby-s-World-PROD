...
export const POST = withAuth(withCsrfProtection(async (request: AuthenticatedRequest) => {
  try {
    ...
    if (!isGranular || amount > 0.02) return NextResponse.json({ error: 'Invalid batch.' }, { status: 400 });

    const newBalance = await executeAddCoin(db, userPublicKey, amount, batchId);

    if (redis && batchId) await redis.set(`idemp:coin:${userPublicKey}:${batchId}`, '1', 'EX', 600);
...
async function checkCoinIdempotency(db: FirebaseFirestore.Firestore, redis: Redis | null, userPublicKey: string, batchId?: string) {
    ...
    if (await redis.get(`idemp:coin:${userPublicKey}:${batchId}`)) {
        ...
        return NextResponse.json({ success: true, newBalance: (doc.data() as PlayerDocument)?.gameUSDTBalance || 0, idempotent: true });
    }
    return null;
}
...
async function handleCoinRateLimit(redis: Redis | null, userPublicKey: string) {
    ...
    if (await redis.get(rlKey)) return NextResponse.json({ error: 'Too many requests.' }, { status: 429 });
    await redis.set(rlKey, '1', 'EX', 1);
    return null;
}

function executeAddCoin(db: FirebaseFirestore.Firestore, userPublicKey: string, amount: number, batchId?: string) {
  ...
  return db.runTransaction(async (transaction) => {
    ...
    logger.log(`[Database] addCoin - User: ${userPublicKey}, Old: ${currentBalance}, Add: ${amount}, New: ${newBalance}, Batch: ${batchId || 'N/A'}`);
