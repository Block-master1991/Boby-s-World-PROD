      ...
      if (redis && batchId) {
        ...
        if (alreadyProcessed) {
          ...
          return NextResponse.json({
            success: true,
            newBalance: (playerDoc.data() as PlayerDocument).gameUSDTBalance,
            idempotent: true,
          });
        }
      }
...
function executePenalty(
  ...
  return db.runTransaction(async transaction => {
    ...
    logger.log(
      `[Database] applyPenalty - User: ${userPublicKey}, Old: ${currentBalance}, Penalty: ${amount}, New: ${newBalance}, Batch: ${batchId || "N/A"}`
    );
