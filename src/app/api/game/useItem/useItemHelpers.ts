 ...
 */
export function validateUseRequest(itemId: unknown, amount: unknown): { itemId: string; amount: number } {
 ...
 */
export function calculateUpdatedInventory(inventory: InventoryItem[], itemId: string, amountToUse: number): InventoryItem[] {
    ...
    if (totalItemCount < amountToUse) {
        throw new Error(`You do not have enough items. You have ${totalItemCount}, but requested ${amountToUse}.`);
    }
