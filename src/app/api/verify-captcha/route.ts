...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!secretKey) {
      logger.error(
        "RECAPTCHA_SECRET_KEY is not set in environment variables. This is a critical configuration error."
      );
      return NextResponse.json(
        { success: false, error: "Server configuration error for CAPTCHA. Secret key is missing." },
        { status: 500 }
      );
    }
    ...
    try {
      ...
      return NextResponse.json(
        { success: false, error: "Invalid response from reCAPTCHA server" },
        { status: 502 }
      );
    }
    ...
    return NextResponse.json(
      { success: false, error: "CAPTCHA verification failed", details: data["error-codes"] },
      { status: 400 }
    );
  } catch (error) {
    logger.error("Error in CAPTCHA verification API:", error);
    return NextResponse.json(
      { success: false, error: "Internal server error during CAPTCHA verification" },
      { status: 500 }
    );
  }
}
