...
const InitiateForm: React.FC<InitiateFormProps> = ({
  email,
  setEmail,
  walletAddress,
  setWalletAddress,
...
const RecoveryCardHeader: React.FC<{ step: "initiate" | "verify" }> = ({ step }) => (
  <CardHeader className="space-y-1">
    ...
    <CardDescription>
      {step === "initiate"
        ? "Enter your wallet address and email to receive a recovery code."
...
const RecoveryCardFooter: React.FC<RecoveryCardFooterProps> = ({
  step,
  loading,
  onRetry,
  onCancel,
  ...
  <CardFooter className="flex flex-col space-y-2 border-t pt-4">
    {step === "verify" && (
      <div className="flex gap-2 w-full">
        <Button variant="outline" size="sm" className="flex-1" onClick={onRetry} disabled={loading}>
          Try different email
        </Button>
        <Button
          variant="destructive"
    ...
    <p className="text-xs text-center text-muted-foreground">
      This process protects your account from unauthorized access.
    </p>
  </CardFooter>
);
...
const useRecoveryHandlers = (toast: ReturnType<typeof useToast>["toast"]) => {
  ...
  const handleInitiate = useCallback(
    async (e: React.FormEvent) => {
      ...
      setLoading(true);
      try {
        const res = await fetchWithCsrf("/api/auth/recovery/initiate", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ email, publicKey: walletAddress }),
        });
        const data = await res.json();
        if (res.ok) {
          setRecoveryToken(data.token);
          setStep("verify");
          toast({ title: "Recovery Email Sent", description: "Check your inbox." });
        } else {
  ...
  const handleVerify = useCallback(
    async (e: React.FormEvent) => {
      ...
      setLoading(true);
      try {
        const res = await fetchWithCsrf("/api/auth/recovery/verify", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ recoveryToken, recoveryCode: code }),
        });
        const data = await res.json();
        if (res.ok) {
          toast({
            title: "Account Recovered",
            description: "Please sign in again with your wallet.",
          });
  ...
  const handleCancel = useCallback(async () => {
    ...
    setLoading(true);
    try {
      await fetchWithCsrf("/api/auth/recovery/verify", {
        method: "DELETE",
        headers: { "Content-Type": "application/json" },
