...
const LoadingScreen = dynamic(() => import('@/components/game-bootstrap/LoadingScreen'), { ssr: false });
...
export const SoundControlButton: React.FC<SoundControlButtonProps> = ({ areSheetsOpen, isSoundPlaying, isMuted, onToggle }) => {
    ...
    return (
        <div style={{ position: 'fixed', top: '20px', right: '20px', zIndex: 1000 }} className="sm:top-6 sm:right-6 md:top-8 md:right-8">
            <Button variant="outline" size="icon" onClick={onToggle} aria-label={!isSoundPlaying ? "Enable Sound" : (isMuted ? "Unmute" : "Mute")}>
                {!isSoundPlaying ? <VolumeX className="h-4 w-4" /> : isMuted ? <VolumeX className="h-4 w-4 text-gray-500" /> : <Volume2 className="h-4 w-4 text-green-500" />}
            </Button>
        </div>
    );
};
...
interface EnableSoundButtonProps { show: boolean; onClick: () => void; }
...
const GameModeContent: React.FC<GameModeContentProps> = ({ selectedGameMode, assetPreloadComplete, onGameModeSelected, onAssetPreloadComplete, onAssetPreloadError }) => {
    if (selectedGameMode !== 'none') return null;
    if (!assetPreloadComplete) return <InitialAssetLoader onComplete={onAssetPreloadComplete} onError={onAssetPreloadError} />;
...
const GameUIContent: React.FC<GameUIContentProps> = (props) => {
    const { selectedGameMode, isGameUILoading, gameUILoadProgress, octreeRef, onSheetsStateChange, onGameUILoadStart, onGameUILoadProgress, onGameUILoadComplete } = props;
    
    if (selectedGameMode === 'boby-world') {
        return (
            <>
                {isGameUILoading && <GameLoadingOverlay isLoading={true} progress={gameUILoadProgress} error={null} phase="Loading game world..." showTips={true} />}
                <GameUI octreeRef={octreeRef} onSheetsStateChange={onSheetsStateChange} onLoadStart={onGameUILoadStart} onLoadProgress={onGameUILoadProgress} onLoadComplete={onGameUILoadComplete} />
            </>
        );
    }
    return (
        <>
            {isGameUILoading && <GameLoadingOverlay isLoading={true} progress={0} error={null} phase="Preparing running game..." showTips={false} />}
...
export const MainContentRenderer: React.FC<MainContentRendererProps> = (props) => {
    ...
    // Admin redirect
    if (isAdminUser) return <LoadingScreen message="Redirecting to admin panel..." showLogo variant="indeterminate" />;

    // Game mode selection
    if (selectedGameMode === 'none') {
        return <GameModeContent selectedGameMode={selectedGameMode} assetPreloadComplete={assetPreloadComplete} onGameModeSelected={onGameModeSelected} onAssetPreloadComplete={onAssetPreloadComplete} onAssetPreloadError={onAssetPreloadError} />;
    }

    // Game UI
    if (isGameUIVisible && (selectedGameMode === 'boby-world' || selectedGameMode === 'running-game')) {
        return <GameUIContent selectedGameMode={selectedGameMode} isGameUILoading={isGameUILoading} gameUILoadProgress={gameUILoadProgress} octreeRef={octreeRef} onSheetsStateChange={onSheetsStateChange} onGameUILoadStart={onGameUILoadStart} onGameUILoadProgress={onGameUILoadProgress} onGameUILoadComplete={onGameUILoadComplete} />;
    }
