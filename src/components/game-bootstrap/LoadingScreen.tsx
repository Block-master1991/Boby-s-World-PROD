...
const loadingTips: LoadingTip[] = [
  ...
  },
  {
    title: "⚡ Speed Boost",
    content:
      "Speedy Paws treats give temporary boosts - perfect for escaping enemies or collecting coins quickly!",
    category: "gameplay",
  },
  {
    title: "🛡️ Guardian Shield",
    content:
      "Guardian Shields protect you from enemy attacks for a short time. Collect them wisely!",
    category: "gameplay",
  },
  {
    title: "🎯 Performance Tips",
    content:
      "For best performance, close other browser tabs and ensure you have at least 4GB RAM available.",
    category: "performance",
  },
  {
    title: "🎮 Animal Friends",
    content:
      "Some animals are friendly collectors, others are sneaky thieves! Learn their behaviors.",
    category: "gameplay",
  },
  {
    title: "🏃‍♂️ Running Game",
    content:
      "Switch between walk mode and run mode - running costs more but lets you escape predators faster.",
    category: "gameplay",
  },
  {
...
const useTimeEstimation = (progress: number, isError: boolean) => {
  ...
  useEffect(() => {
    ...
    setTimeLeft(prev =>
      prev === null ? Math.round(remaining) : Math.round(prev * 0.7 + remaining * 0.3)
    );
...
const useLoadingTips = (showTips: boolean) => {
  ...
  useEffect(() => {
    if (!showTips) return;
    const update = () =>
      setCurrentTip(loadingTips[Math.floor(Math.random() * loadingTips.length)] || null);
    update();
    ...
    {[...Array(5)].map((_, i) => (
      <div
        ...
        style={{
          ...
          top: `${Math.random() * 100}%`,
          animation: `floatParticle${(i % 3) + 1} ${8 + Math.random() * 6}s infinite`,
        }}
      />
    ))}
    {[...Array(12)].map((_, i) => (
      <div
        ...
        style={{
          left: `${Math.random() * 100}%`,
          top: `${Math.random() * 100}%`,
          animation: `floatParticle${(i % 3) + 1} ${4 + Math.random() * 4}s infinite`,
        }}
      />
    ))}
    {[...Array(20)].map((_, i) => (
...
const LogoSection = ({ isError }: { isError: boolean }) => (
  <div className="relative mb-8 z-10">
    <div
      ...
      style={{ animation: !isError ? "logoFloat 6s infinite" : undefined }}
    >
      <Image
        src="/Boby-logo.png"
        alt="Logo"
    ...
    {!isError && (
      <div
        ...
        style={{ animation: "pawOrbit 8s linear infinite", transformOrigin: "60px 60px" }}
      >
...
const PhaseSection = ({
  ...
  <div className="flex flex-col items-center mb-6">
    <div
      className={`h-16 w-16 rounded-full bg-primary/10 flex items-center justify-center animate-pulse ${phase === "world" ? "animate-bounce" : ""}`}
    >
      {info.icon}
    </div>
  ...
  <div className="flex flex-col items-center mb-6">
    ...
    <Card className="bg-destructive/10 border-destructive/20 mt-6 p-4">
      <button
        ...
        className="text-xs bg-destructive text-primary-foreground px-3 py-1 rounded shadow-sm hover:bg-destructive/90 transition-colors"
      >
        Refresh Page
      </button>
    </Card>
...
const MainContent: React.FC<ContentProps> = ({
  ...
  <>
    {variant !== "indeterminate" && (
      <PhaseSection info={phaseInfo} message={message} phase={phase} />
    )}
    {variant === "default" && (
      <Card className="mb-6 bg-card/50 backdrop-blur-sm border-primary/20">
        <CardContent className="p-4 space-y-2">
          ...
          {estimatedTime !== null && estimatedTime > 0 && (
            <div className="text-xs text-primary/80 mt-1">⏱️ {estimatedTime} seconds left</div>
          )}
        </CardContent>
      </Card>
    )}
    {currentAsset && loadedAssets !== undefined && totalAssets !== undefined && (
      <Card className="mb-4 bg-card/40 border-primary/10 p-3">
        <div className="flex justify-between text-xs text-muted-foreground">
      ...
      <Card className="bg-card/30 border-primary/10 p-4 shadow-sm">
        <div className="flex items-start gap-3">
          <div className="text-lg">
            {(
              { gameplay: "🎮", world: "🌍", performance: "⚡", fun: "🎯" } as Record<
                string,
                string
              >
            )[currentTip.category] || "💡"}
          ...
          <div className="text-left">
            <h3 className="text-sm font-semibold text-primary">{currentTip.title}</h3>
...
const LoadingScreen: React.FC<LoadingScreenProps> = ({
  ...
  const phaseInfo = useMemo(
    () => loadingPhases.find(p => p.id === phase) || loadingPhases[1]!,
    [phase]
  );
  ...
  if (variant === "indeterminate") {
    return (
      <div className="game-bootstrap-container flex flex-col items-center justify-center min-h-screen bg-background text-center p-6">
        ...
        {message && (
          <p className="mt-6 text-muted-foreground text-sm font-medium tracking-wide animate-pulse">
            {message}
          </p>
        )}
      </div>
