...
const useCaptchaScriptLoader = (toast: ToastFn, setTheme: React.Dispatch<React.SetStateAction<Theme>>) => {
  ...
  const handleError = useCallback(() => {
    ...
    setAttempts(prev => {
      ...
      if (next >= 3) {
        ...
        toast({ title: 'Loading Error', description: 'Failed to load verification system.', variant: 'destructive' });
      } else {
  ...
  const retry = useCallback(() => {
    setIsLoading(true); setAttempts(0); setShowRetry(false); setError(null);
    setTheme(t => t === 'light' ? 'dark' : 'light');
  }, [setTheme]);
...
const useCaptchaVerification = (onSuccess: () => void, toast: ToastFn) => {
  ...
  const verify = useCallback(async (token: string, reset: () => void) => {
    ...
    try {
      ...
      toast({ title: 'Network Error', description: 'Verification failed.', variant: 'destructive' });
      reset();
    } finally {
...
const CaptchaHeader = () => (
  <>
    ...
    <p className="text-xl text-muted-foreground mb-6 max-w-md">Please complete the verification below.</p>
  </>
);
...
const CaptchaStatus: React.FC<StatusProps> = ({ isVerifying, showRetry, loadErr, verifyErr, onRetry }) => (
  <div className="mt-4 flex flex-col items-center">
    {isVerifying && <div className="flex items-center text-muted-foreground"><PawPrint className="mr-2 h-5 w-5 animate-pulse" /><span>Verifying...</span></div>}
    {showRetry && (
      <div className="flex flex-col items-center">
        <p className="text-sm text-destructive mb-2 text-center">Having trouble loading verification?</p>
        <button onClick={onRetry} className="px-4 py-2 bg-primary text-primary-foreground rounded-md hover:bg-primary/90 transition-colors text-sm">Try Again</button>
      </div>
    )}
...
const CaptchaScreen: React.FC<CaptchaScreenProps> = ({ siteKey, onVerificationSuccess }) => {
  ...
  const { isLoading, attempts, showRetry, error: loadErr, handleLoad, handleError, retry } = useCaptchaScriptLoader(toast, setTheme);
  const { isVerifying, error: verifyErr, verify, setError: setVerifyErr } = useCaptchaVerification(onVerificationSuccess, toast);
  ...
  return (
    <div className="relative flex flex-col items-center justify-center min-h-screen bg-background text-foreground p-8 text-center">
      ...
      <div className="mb-4 p-4 bg-card rounded-lg shadow-md border border-border relative">
        <ReCAPTCHA key={`${theme}-${attempts}`} ref={recaptchaRef} sitekey={siteKey} theme={theme} hl="en"
          onChange={(t) => { if (t) { setVerifyErr(null); verify(t, () => recaptchaRef.current?.reset()); } }}
          asyncScriptOnLoad={handleLoad} onErrored={handleError} />
        {isLoading && (
          <div className="absolute inset-0 bg-card/90 backdrop-blur-sm flex flex-col items-center justify-center rounded-md">
            <PawPrint className="h-8 w-8 animate-pulse text-primary mb-4" /><p className="text-sm text-muted-foreground">Loading verification system...</p>
            {attempts > 0 && <p className="text-xs text-muted-foreground mt-2">Attempt {attempts}/3</p>}
          </div>
        )}
      </div>
      <CaptchaStatus isVerifying={isVerifying} showRetry={showRetry} loadErr={loadErr} verifyErr={verifyErr} onRetry={retry} />
    </div>
  );
};
