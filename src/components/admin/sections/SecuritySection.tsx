...
export function SecuritySection() {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      <Card>
        ...
        <CardContent className="space-y-6">
          ...
          <IpControls
            ...
            newIp={logic.newIp} setNewIp={logic.setNewIp}
            targetList={logic.targetList} setTargetList={logic.setTargetList}
            loading={logic.loading} onAddIp={logic.handleAddIp}
            resetPagination={logic.resetPagination}
          />
          <IpManagementGrid
            whitelist={logic.whitelist} blacklist={logic.blacklist}
            loading={logic.loading} whitePage={logic.whitePage}
            blackPage={logic.blackPage} PAGE_SIZE={logic.PAGE_SIZE}
            getPaginatedList={logic.getPaginatedList} setWhitePage={logic.setWhitePage}
      ...
      {logic.confirmDelete && (
        <DeleteConfirmModal
          ip={logic.confirmDelete.ip} list={logic.confirmDelete.list}
          loading={logic.loading} onCancel={() => logic.setConfirmDelete(null)}
...
function SecurityMessage({ message }: { message: { type: 'success' | 'error'; text: string } | null }) {
