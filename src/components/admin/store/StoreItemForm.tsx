  ...
  <div className="grid grid-cols-2 gap-6 py-2">
    <FormField id="id" label="ID" hint="Fixed ID used in game logic">
      <Input
        id="id"
        value={f.id}
        onChange={e => s({ ...f, id: e.target.value })}
        disabled={id}
    ...
    <FormField id="price" label="Price (USD)">
      <Input
        ...
        step="0.001"
        value={f.price}
        onChange={e => s({ ...f, price: parseFloat(e.target.value) || 0 })}
      />
    </FormField>
    <div className="space-y-2">
      <Label className="text-xs font-bold uppercase tracking-wider text-muted-foreground">
      ...
      <div className="flex gap-2">
        ...
        <label className="cursor-pointer shrink-0">
          ...
          <input
            ...
            className="hidden"
            accept="image/*"
            onChange={e => e.target.files?.[0] && oiu(e.target.files[0])}
            disabled={u}
          />
        </label>
      </div>
    ...
    <SelectField<ItemType>
      label="Type"
      value={f.type}
      onValueChange={v => s({ ...f, type: v })}
      options={["consumable", "permanent"]}
    />
    <SelectField<ItemRarity>
      label="Rarity"
      value={f.rarity}
      onValueChange={v => s({ ...f, rarity: v })}
