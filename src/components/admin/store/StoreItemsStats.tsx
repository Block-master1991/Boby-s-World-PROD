...
export const StoreItemsStats: React.FC<StoreItemsStatsProps> = ({ items, loading }) => {
  ...
  const stats = [
    ...
    {
      label: "Consumable",
      value: items.filter(i => i.type === "consumable").length,
      dot: "amber",
      color: "amber",
    },
    {
      label: "Permanent",
      value: items.filter(i => i.type === "permanent").length,
      dot: "purple",
      color: "purple",
    },
  ];
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
      {stats.map((s, i) => (
        ...
        >
          <div
            className={`absolute top-0 right-0 w-16 h-16 bg-gradient-to-br from-${s.color}-500/10 to-transparent rounded-bl-full`}
          ></div>
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              <div
                className={`flex h-10 w-10 items-center justify-center rounded-lg bg-${s.color}-500/10`}
              >
                {s.icon ? (
                  <s.icon className={`h-5 w-5 text-${s.color}-500`} />
                ) : (
                  <div className={`h-3 w-3 bg-${s.dot}-500 rounded-full`} />
                )}
              </div>
              <div>
                <p className={`text-2xl font-bold ${s.dot ? `text-${s.dot}-500` : ""}`}>
                  {s.value}
                </p>
