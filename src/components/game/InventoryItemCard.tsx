...
const InventoryItemCard: React.FC<InventoryItemCardProps> = ({ definition, currentCount, isLoading, onUse }) => {
    const [quantity, setQuantity] = useState(1);
    const isAutomated = definition.id === '1'; // Protection Bottle

    const handleQuantityChange = useCallback((val: number) => setQuantity(Math.max(1, Math.min(val, currentCount))), [currentCount]);
    ...
    return (
        <Card className="flex flex-col">
            <CardHeader className="flex-row items-center gap-3 p-4 space-y-0">
                <Image src={definition.image || 'https://placehold.co/60x60.png'} alt={definition.name} width={48} height={48} className="rounded-md border" />
                <div>
            ...
            {definition.description && (
                <CardContent className="p-4 pt-0 flex flex-col flex-grow">
                    ...
                    {!isAutomated && currentCount > 0 && (
                        <>
                            <ItemQuantitySelector quantity={quantity} maxCount={currentCount} isDisabled={isLoading} onIncrement={handleIncrement} onDecrement={handleDecrement} onMax={handleMax} onChange={handleQuantityChange} />
                            <Button variant="default" size="sm" className="mt-4 w-full text-xs px-2 py-1" onClick={() => onUse(definition.id, quantity)} disabled={isLoading}>
