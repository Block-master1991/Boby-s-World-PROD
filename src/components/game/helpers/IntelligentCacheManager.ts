...
export class IntelligentCacheManager {
  private cache = new Map<
    string,
    { data: unknown; timestamp: number; accessCount: number; size: number }
  >();
  ...
  async get(
    ...
    if (cached) {
      ...
      this.stats.hits++;
      logger.log(
        `[CacheManager] Cache hit for ${key} (size: ${(cached.size / 1024).toFixed(1)}KB)`
      );
      return cached.data;
    }
    ...
    try {
      ...
      logger.log(
        `[CacheManager] Cache miss for ${key}, loaded and cached (${(estimatedSize / 1024).toFixed(1)}KB)`
      );
      return data;
    } catch (error) {
  ...
  private async store(key: string, data: unknown, size: number): Promise<void> {
    ...
    this.updateAccessOrder(key);
    logger.log(
      `[CacheManager] Stored ${key} in cache. Total cached: ${this.cache.size} items, ${(this.currentSize / 1024 / 1024).toFixed(1)}MB`
    );
  }
  ...
  private async evictToFit(neededSpace: number): Promise<void> {
    ...
    while (
      this.currentSize + spaceToFree > this.maxSize &&
      this.cache.size > 0 &&
