...
export class PriorityAssetLoader {
    ...
    private async resolveDependencies(entry: AssetManifestEntry, loadingStack: Set<string> = new Set()): Promise<void> {
        ...
        await Promise.all(entry.dependencies.map(async (depId) => {
    ...
    async loadAssetsByPriority(onProgress: LoadingProgressCallback): Promise<void> {
        ...
        if (assetsByPriority[AssetPriority.HIGH]) {
            loadedCount = await this.loadAssetsInParallel({
                assets: assetsByPriority[AssetPriority.HIGH], onProgress, phase: 'graphics',
                startRange: 25, endRange: 50, totalAssets, currentLoaded: loadedCount
            });
        }

        if (assetsByPriority[AssetPriority.MEDIUM]) {
            loadedCount = await this.loadAssetsInParallel({
                assets: assetsByPriority[AssetPriority.MEDIUM], onProgress, phase: 'audio',
                startRange: 50, endRange: 75, totalAssets, currentLoaded: loadedCount
            });
        }

        if (assetsByPriority[AssetPriority.LOW]) {
            Promise.all(assetsByPriority[AssetPriority.LOW].map(asset => this.loadAsset(asset, onProgress)))
                .catch(logger.error);
        }
    }
    ...
    private groupAssetsByPriority(): { [key in AssetPriority]: AssetManifestEntry[] } {
        const grouped: { [key in AssetPriority]: AssetManifestEntry[] } = {
            [AssetPriority.CRITICAL]: [], [AssetPriority.HIGH]: [],
            [AssetPriority.MEDIUM]: [], [AssetPriority.LOW]: []
        };
        this.manifest.forEach(asset => grouped[asset.priority].push(asset));
        return grouped;
    }

    private async loadAsset(asset: AssetManifestEntry, _onProgress?: LoadingProgressCallback): Promise<void> {
        if (this.loadedAssets.has(asset.id)) return;
        try {
            if (asset.dependencies) {
                await Promise.all(asset.dependencies.map(async (dep) => {
