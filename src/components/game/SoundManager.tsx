...
const getTrackForScreen = (screen: ScreenType | null): string | null => {
  ...
  switch (screen) {
    ...
    case "loading":
      return preGame;
    case "boby-world":
      return "/audio/Boby_On_the_Run_open_world_bg_sound.mp3";
    case "running-game":
      return "/audio/Boby_On_the_Run_road_run_bg_sound.mp3";
    default:
      return null;
  }
};
...
const useTrackAutoPlayer = (ops: AutoPlayerOptions) => {
  ...
  const setAudioSource = useCallback(
    async (src: string, loop: boolean = true) => {
  ...
  useEffect(() => {
    ...
    if (track) setAudioSource(track);
      ...
      currentOriginalSrc.current = null;
    }
