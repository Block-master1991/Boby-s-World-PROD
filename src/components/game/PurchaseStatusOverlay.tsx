        ...
        {phases.slice(0, -1).map((p, index) => {
          ...
          return (
            <React.Fragment key={p}>
              <div
                className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold transition-all duration-300 ${isComplete ? "bg-green-500 text-white" : isActive ? "bg-primary text-primary-foreground animate-pulse" : isError ? "bg-red-500 text-white" : "bg-muted text-muted-foreground"}`}
              >
                {isComplete ? "✓" : index + 1}
              </div>
