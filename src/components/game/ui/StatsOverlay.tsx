...
export const StatsOverlay: React.FC<StatsOverlayProps> = ({
  ...
  return (
    <div
      className={`absolute top-[calc(1rem+var(--sat))] left-1/2 -translate-x-1/2 z-20 bg-primary/80 text-primary-foreground px-4 py-2 rounded-lg shadow-lg flex flex-col md:flex-row items-center md:justify-center space-y-1 md:space-y-0 md:space-x-4 ${isWalletMismatch ? "mt-12" : ""}`}
    >
      <div className="flex items-center">
        <Image
          ...
          width={20}
          height={20}
          className="h-5 w-5 mr-2 rtl:ml-2"
        />
      ...
      <div className="flex items-center text-xs opacity-90">
        ...
        <span>
          Coins: {remainingCoinsOnMap} / {COIN_COUNT}
        </span>
      </div>
      {protectionBottleDef && (
        <div className="flex items-center text-xs opacity-90">
