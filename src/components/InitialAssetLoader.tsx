...
const getPhaseText = (p: string) => {
  const map: Record<string, string> = {
    ...
    completed: "Done!",
    failed: "Failed",
    "Loading critical priority assets": "Loading Core...",
    "Loading high priority assets": "Loading High...",
    "Loading medium priority assets": "Loading Medium...",
    "Loading low priority assets": "Loading Additional...",
  };
  return map[p] || p;
};

const AssetLoaderUI: React.FC<{ progress: PreloadProgress; eta: number | null }> = ({
  progress,
  eta,
}) => {
    ...
    <Card className="w-full max-w-md md:max-w-2xl glass-card">
      ...
      <CardContent className="space-y-6">
        ...
        <div className="grid grid-cols-2 gap-4 text-sm text-center">
          <div>
            <div className="font-semibold">{progress.loadedAssets}</div>
            <div className="text-muted-foreground">of {progress.totalAssets} Files</div>
          </div>
          <div>
            <div className="font-semibold">{formatSize(progress.loadedSizeMB)}</div>
            <div className="text-muted-foreground">of {formatSize(progress.totalSizeMB)} Size</div>
          </div>
        </div>
        ...
        {eta !== null && !progress.isComplete && (
          <div className="text-center text-sm text-muted-foreground">
            ...
            {progress.downloadSpeed > 0 && (
              <span className="block text-xs">Speed: {progress.downloadSpeed.toFixed(2)} MB/s</span>
            )}
          </div>
        )}
        {progress.verifiedAssets > 0 && (
          <div className="bg-accent/20 border border-accent/30 rounded-lg p-3 text-xs flex justify-between">
        ...
        {progress.errors.length > 0 && (
          <div className="bg-destructive/10 border border-destructive/20 rounded-lg p-3 text-xs text-destructive">
            <div className="font-medium mb-1">Errors ({progress.errors.length})</div>
...
const InitialAssetLoader: React.FC<InitialAssetLoaderProps> = ({ onComplete, onError }) => {
  const { progress, estimatedTimeRemaining, isCheckOnly, isInitialLoading } = useInitialAssetLoader(
    { onComplete, onError }
  );
  ...
  return (
    <div className="min-h-screen bg-background text-foreground px-4 flex items-center justify-center">
      <AssetLoaderUI progress={progress} eta={estimatedTimeRemaining} />
    </div>
  );
};
