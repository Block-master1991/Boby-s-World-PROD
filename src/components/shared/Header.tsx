...
const SecurityBadge: React.FC<SecurityBadgeProps> = ({ level }) => {
  ...
  const icon = level === 'Maximum' ? <Zap size={12} /> : 
               level === 'Enhanced' ? <Shield size={12} /> : 
               <CheckCircle2 size={12} />;

  return (
    <div className={`flex items-center gap-1.5 px-2 py-0.5 rounded-full border text-[10px] font-bold uppercase tracking-wider ${colors[level] || colors['Standard']}`}>
      {icon}
      {level} Protection
    </div>
  );
...
const LogoSection = ({ isClient, securityLevel, isOnline }: { isClient: boolean, securityLevel: string, isOnline: boolean }) => (
