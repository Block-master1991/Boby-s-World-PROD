...
async function handleRateLimitExceeded(
  ...
  logger.warn(
    `[GraphQL-RateLimit] Mutation ${mutationName} rate limit exceeded for ${identifier}. Count: ${limit}/${limit}`
  );
  return {
    allowed: false,
    remaining: 0,
...
export async function checkGraphQLMutationRateLimit(
  ...
  try {
    ...
    if (count >= limit) {
      return await handleRateLimitExceeded({
        mutationName,
        identifier,
        redisKey,
        limit,
    ...
    logger.log(
      `[GraphQL-RateLimit] Mutation ${mutationName} allowed for ${identifier}. Count: ${count + 1}/${limit}, Remaining: ${remaining}`
    );
 ...
 */
export function getMutationRateLimitConfigs(): Record<
  string,
