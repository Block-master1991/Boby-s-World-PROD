 ...
 */
function detectAnomalies(patternData: { userAgent?: string; endpoints: Record<string, number[]> }, userAgent: string): { type: PatternAnalysisResult['type'], desc: string, score: number, flags: Record<string, boolean> } {
    const uniqueEndpoints = Object.keys(patternData.endpoints).length;
    const hasConcentration = Object.values(patternData.endpoints).some((arr) => Array.isArray(arr) && arr.length > 20);
    ...
    if (flags.tooManyEndpoints && hasConcentration) {
        return { type: 'unusual_pattern', desc: 'Multiple endpoint reconnaissance attempt', score: 25, flags };
    }
...
export async function analyzeRequestPatterns(
    ...
    try {
        ...
        const patternData = rawPatterns ? JSON.parse(rawPatterns) : {
        ...
        return { type: 'rapid_requests', description: 'Error in pattern analysis', evidence: null, score: 0 };
    }
}
...
export function analyzeBehavior(
    ...
    if (stats.requestCount > 10) {
        ...
        stats.avgFrequency = stats.requestCount === 1 ? initialFreq : (stats.avgFrequency + initialFreq) / 2;
    }
