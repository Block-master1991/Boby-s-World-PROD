...
try {
  ...
  if (isNode) {
    ...
    const req = (
      globalThis as unknown as {
        require?: (id: string) => { AsyncLocalStorage: typeof AsyncLocalStorageConstructor };
      }
    ).require;
  ...
  AsyncLocalStorageConstructor = class MockAsyncLocalStorage<
    T,
  > implements AsyncLocalStorageLike<T> {
...
export class ContextManager {
  ...
  extractFromHeaders(headers: Headers | Record<string, string>): Partial<LogContext> {
    ...
    const correlationId =
      getHeader("x-correlation-id") || getHeader("x-request-id") || getHeader("request-id");
    if (correlationId) {
