...
export class LogEncryption implements ILogEncryption {
  ...
  private initializeMasterKey(): void {
    ...
    if (envKey) {
      ...
      if (typeof window === "undefined") {
        ...
        console.warn(
          "[LogEncryption] No LOG_ENCRYPTION_KEY found. Generating temporary key. THIS IS NOT SECURE FOR PRODUCTION!"
        );
      }
  ...
  private deriveKey(secret: string, salt?: Buffer | Uint8Array): Buffer | Uint8Array | null {
    ...
    if (this.config.keyDerivation === "direct") {
      return cryptoModule.createHash
        ? cryptoModule.createHash("sha256").update(secret).digest()
        : null;
    }
    ...
    if (!cryptoModule.pbkdf2Sync) {
      // eslint-disable-next-line no-console
      console.warn(
        "[LogEncryption] pbkdf2Sync not available in this environment. Encryption disabled."
      );
      return null;
    }
  ...
  encryptFields<T>(data: T): T {
    ...
    const result = (Array.isArray(data) ? [...data] : { ...data }) as
  ...
  private shouldEncryptField(fieldName: string): boolean {
    ...
    return (
      this.config.encryptedFields?.some(field => normalized.includes(field.toLowerCase())) || false
    );
  }
