...
export class RateLimitMiddleware {
    ...
    checkLimit(userId?: string, endpoint?: string): Promise<RateLimitResult> {
        if (!this.config.enabled) {
            return Promise.resolve({ allowed: true, remaining: Infinity, resetAt: 0, identifier: 'disabled' });
        }
    ...
    private checkUserLimit(userId: string): RateLimitResult {
        return this.processEntryCount({
            ...
            setter: (e) => { this.userLimits.set(userId, e); },
            originalId: userId,
            type: 'user'
        });
    }
    ...
    private checkEndpointLimit(endpoint: string): RateLimitResult {
        return this.processEntryCount({
            ...
            setter: (e) => { this.endpointLimits.set(endpoint, e); },
            originalId: endpoint,
            type: 'endpoint'
        });
    }
    ...
    private processEntryCount(opts: {
        ...
        return { allowed: true, remaining: cfg.max - entry.count, resetAt: entry.resetAt, identifier: id };
    }
...
export const rateLimitMiddleware = new RateLimitMiddleware({
    ...
    onLimitExceeded: (id, type) => {
        professionalLogger.warn(`[RateLimit] Limit exceeded for ${type}: ${id}`, { rateLimit: { id, type } });
    }
});
