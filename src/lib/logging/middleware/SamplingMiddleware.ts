...
export class SamplingMiddleware {
   ...
   * Add priority rule
   */
  addPriorityRule(condition: (entry: SampledLogEntry) => boolean, rate: number): void {
    if (!this.config.priorityRules) {
  ...
  updateRate(level: string, rate: number): void {
    ...
    if (validLevels.includes(level)) {
      ...
      switch (level as keyof NonNullable<SamplingConfig["rates"]>) {
        case "trace":
          this.config.rates.trace = rate;
          break;
        case "debug":
          this.config.rates.debug = rate;
          break;
        case "info":
          this.config.rates.info = rate;
          break;
        case "warn":
          this.config.rates.warn = rate;
          break;
        case "error":
          this.config.rates.error = rate;
          break;
        case "fatal":
          this.config.rates.fatal = rate;
          break;
      }
    }
  }
 ...
 */
export function createSamplingWithRules(config?: Partial<SamplingConfig>): SamplingMiddleware {
