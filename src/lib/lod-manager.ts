...
interface DitherableMaterial extends Material {
...
interface LODObject {
  ...
  morphSpeed: number;
  cachedMaterials?: DitherableMaterial[]; // Cache for performance
  initialFadeFinished: boolean;
  _ditherValue?: number;
}
...
export class LODManager {
  ...
  private updateInterval = 300; // Optimized: 300ms check is sufficient for LOD
  ...
  registerLODObject(id: string, object: Object3D, levels: Omit<LODLevel, "visible">[]): void {
    ...
    this.lodObjects.set(id, {
      ...
      morphProgress: 0,
      morphSpeed: 2.0,
      initialFadeFinished: false,
    });

    // Start with dither hidden if it supports it
    this.setInitialDitherState(this.lodObjects.get(id)!);
  }

  private setInitialDitherState(lodObject: LODObject): void {
    lodObject.object.traverse(child => {
      if (child instanceof Mesh) {
        const m = child.material as DitherableMaterial;
        if (m.setDitherIntensity) m.setDitherIntensity(0.0);
      }
    });
  }
  ...
  update(deltaTime: number): void {
    ...
    if (currentTime - this.lastUpdateTime >= this.updateInterval) {
      ...
      for (const [id, lodObject] of this.lodObjects) {
        if (!lodObject.object.visible && lodObject.initialFadeFinished) continue;
        this.updateLODObject(id, lodObject);
      }
    }
    this.updateMorphing(deltaTime);
    this.updateInitialFades(deltaTime);
    ...
    if (currentTime - this.lastDiagTime > 5000) {
      ...
      logger.log(
        `[PerfDiag] LODManager: ${avg.toFixed(3)}ms | Objects: ${this.lodObjects.size} | Morphing: ${this.morphingObjects.size}`
      );
      this.totalLODTime = 0;
      this.lodEntries = 0;
  ...
  private updateMorphing(deltaTime: number): void {
    for (const id of this.morphingObjects) {
      ...
      if (lodObject.morphProgress >= 1.0) {
        ...
        delete lodObject.cachedMaterials;
        this.morphingObjects.delete(id);
  ...
  private updateInitialFades(deltaTime: number): void {
    const fadeSpeed = 1.5;
    for (const lodObject of this.lodObjects.values()) {
      if (lodObject.initialFadeFinished) continue;

      const mats = this.getOrCacheMaterials(lodObject);
      let allFinished = true;

      mats.forEach(m => {
        if (m.setDitherIntensity) {
          const current = lodObject._ditherValue || 0;
          const next = Math.min(1.0, current + deltaTime * fadeSpeed);
          m.setDitherIntensity(next);
          lodObject._ditherValue = next;
          if (next < 1.0) allFinished = false;
        }
      });

      if (allFinished) {
        lodObject.initialFadeFinished = true;
        // Final pass to ensure clean state
        mats.forEach(m => m.setDitherIntensity?.(1.0));
      }
    }
  }

  private getOrCacheMaterials(lodObject: LODObject): DitherableMaterial[] {
    if (!lodObject.cachedMaterials) {
      lodObject.cachedMaterials = [];
      lodObject.object.traverse(child => {
        if (child instanceof Mesh && child.material) {
          const mats = Array.isArray(child.material) ? child.material : [child.material];
          lodObject.cachedMaterials!.push(...(mats as DitherableMaterial[]));
        }
      });
    }
    return lodObject.cachedMaterials;
  }
  ...
  private updateMorphProgress(lodObject: LODObject, progress: number): void {
    ...
    const mats = this.getOrCacheMaterials(lodObject);
    mats.forEach(mat => {
      const m = mat as DitherableMaterial & { opacity?: number; transparent?: boolean };
      if (m && "opacity" in m) {
        m.opacity = progress;
