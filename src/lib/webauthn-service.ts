...
export class WebAuthnService {
   ...
   */
  public static getChallengeKey(
    type: "registration" | "auth" | "discovery",
    identifier: string
  ): string {
    switch (type) {
      case "registration":
        return `webauthn_registration_challenge:${identifier}`;
      case "auth":
        return `webauthn_auth_challenge:${identifier}`;
      case "discovery":
        return `webauthn_discovery_challenge:${identifier}`;
      default:
   ...
   */
  public static async initiateRegistration(
    userId: string,
    userName: string,
    rpId: string,
  ...
  public static async completeRegistration(
    userId: string,
    credential: { id: string; publicKey: string; authData?: string | undefined },
    description?: string,
    transports: string[] = []
  ) {
    const aaguid = credential.authData
      ? WebAuthnUtils.extractAAGUID(credential.authData)
      : undefined;
  ...
  public static async listUserPasskeys(userId: string) {
    ...
    return passkeysSnapshot.docs.map(doc => {
      ...
      return {
        id: doc.id,
        credentialId: data["credentialId"] || doc.id,
   ...
   */
  public static async removePasskey(
    userId: string,
    credentialId: string,
    metadata: AuditEventMetadata
  ) {
    ...
    if (passkeysSnapshot.size <= 1) {
      throw new Error(
        "Cannot delete the last passkey. Please set up account recovery or another passkey first."
      );
    }
