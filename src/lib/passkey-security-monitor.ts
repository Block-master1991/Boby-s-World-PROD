...
export class PasskeySecurityMonitor {
   ...
   */
  public async logPasskeyRegistration(
    userId: string,
   ...
   */
  public async logPasskeyLoginSuccess(
    userId: string,
    deviceInfo?: PasskeyDeviceInfo
  ): Promise<void> {
    await auditLogger.logEvent(
   ...
   */
  public async logPasskeyLoginFailure(
    userId: string,
    reason: string,
    deviceInfo?: PasskeyDeviceInfo
  ): Promise<void> {
  ...
  public async logSuspiciousActivity(
    ...
    await sendSlackAlert(`Suspicious passkey activity detected: ${activity}`, {
      level: "critical",
