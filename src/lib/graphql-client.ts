...
const tryRefreshToken = async (authHeaders: Record<string, string>): Promise<boolean> => {
  try {
    ...
    logger.warn("[GraphQL] Token refresh failed:", e);
  }
  return false;
};
...
export const getGraphQLClient = (): Client => {
  if (!graphqlClient) graphqlClient = createGraphQLClient();
  return graphqlClient;
};

// --- Query/Mutation Wrappers ---
const buildErrorResponse = <T>(
  errors: GraphQLError[],
...
export const graphqlQuery = async <T = unknown>(
  query: string,
  variables?: Variables,
  options?: { requestPolicy?: RequestPolicy; context?: Record<string, unknown> }
  ...
  try {
    ...
    if (result.error) {
      const errors =
        result.error.graphQLErrors?.map(
          err => new GraphQLError(err.message, err.extensions as GraphQLErrorExtensions)
        ) || [];
...
export const graphqlMutation = async <T = unknown>(
  ...
  try {
    const result = await client
      .mutation(mutation, variables || {}, { ...options?.context })
      .toPromise();
    if (result.error) {
      const errors =
        result.error.graphQLErrors?.map(
...
export const graphqlUtils = {
  clearCache: () => {
    logger.log("[GraphQL] Cache clearing not implemented");
  },
  getCacheStats: () => ({ operations: 0, cacheSize: 0, hitRate: 0 }),
  healthCheck: async (): Promise<boolean> => {
    try {
      const r = await graphqlQuery(`query HealthCheck { health }`);
      return !r.errors && (r.data as { health?: string })?.health === "OK";
    } catch {
      return false;
    }
  },
