...
export async function verifyAssetIntegrity(
  ...
  try {
    ...
    if (check.isValid) {
      logger.log(
        `[AssetIntegrity] ✓ Verified: ${path} (${(data.byteLength / (1024 * 1024)).toFixed(2)}MB)`
      );
    }
  } catch (error) {
    check.isValid = false;
...
export async function verifyMultipleAssets(
  ...
  logger.log(
    `[AssetIntegrity] Verification complete in ${duration.toFixed(2)}s: ${report.passed} passed, ${report.failed} failed`
  );
...
export function verifySizeOnly(
  ...
  return {
    ...
    lastChecked: Date.now(),
    error: !isValid
      ? `Size difference ${diffMB.toFixed(3)}MB exceeds tolerance ${toleranceMB}MB`
      : undefined,
  };
}
...
export function getCachedIntegrityCheck(
  path: string,
...
export function clearIntegrityCache(): void {
  ...
  Object.keys(localStorage).forEach(k => {
    if (k.startsWith("integrity_")) localStorage.removeItem(k);
  });
...
export function getIntegrityStats() {
  if (typeof window === "undefined")
    return { cachedChecks: 0, oldestCheck: null, newestCheck: null };
  const keys = Object.keys(localStorage).filter(k => k.startsWith("integrity_"));
  const times = keys
    .map(k => JSON.parse(localStorage.getItem(k) || "{}").timestamp)
    .filter(Boolean);
  return {
