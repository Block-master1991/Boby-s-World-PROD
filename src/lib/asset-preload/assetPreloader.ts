...
class IntelligentAssetPreloader {
  ...
  private calculatePreloadZones(
    px: number,
    pz: number,
    v: { x: number; z: number }
  ): PreloadZone[] {
    const zones: PreloadZone[] = [
      { centerX: px, centerZ: pz, radius: 20, assets: [], priority: 10 },
    ];
    const speed = Math.sqrt(v.x ** 2 + v.z ** 2);
    if (speed > 0.1) {
      zones.push({
        centerX: px + (v.x / speed) * this.preloadDistance,
        centerZ: pz + (v.z / speed) * this.preloadDistance,
        radius: 30,
        assets: [],
        priority: 8,
      });
  ...
  private cleanupDistantAssets(px: number, pz: number): void {
    ...
    for (const [id, asset] of this.assets) {
      ...
      const d = Math.sqrt(
        (asset.chunkCoords.x * 16 - px) ** 2 + (asset.chunkCoords.z * 16 - pz) ** 2
      );
  ...
  private evictOldAssets(): void {
    const sorted = Array.from(this.loadedAssets)
      .map(id => ({ id, a: this.assets.get(id)! }))
      .sort((a, b) => a.a.lastAccessed - b.a.lastAccessed);
    let freed = 0;
