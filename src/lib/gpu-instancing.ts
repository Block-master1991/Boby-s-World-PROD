...
import type { Camera } from "@/lib/three-chunk";
import {
  Box3,
  Euler,
  Frustum,
...
class GPUInstancingManager {
  ...
  createInstancedObject(id: string, config: InstancedObjectConfig): InstancedMesh {
    const instancedMesh = new InstancedMesh(config.geometry, config.material, config.maxInstances);
...
export const createTreeInstances = (
  positions: Vector3[],
  scales: Vector3[],
  rotations: Euler[]
): InstanceData[] => {
