...
export async function getStoreItemFromFirestore(
  itemId: string
): Promise<StoreItemDefinition | null> {
  try {
...
export async function getAllStoreItems(): Promise<StoreItemDefinition[]> {
  try {
    await initializeAdminApp();
    const db = getFirestore();
    const itemsSnapshot = await db.collection("storeItems").orderBy("createdAt", "desc").get();
...
export async function getActiveStoreItems(): Promise<StoreItemDefinition[]> {
  try {
    await initializeAdminApp();
    const db = getFirestore();
    const itemsSnapshot = await db.collection("storeItems").where("isActive", "==", true).get();
