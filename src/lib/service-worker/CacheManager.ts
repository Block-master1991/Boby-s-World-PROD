...
export class AdvancedCacheManager {
    ...
    private initializeCacheConfigs(): void {
        this.addCache('critical', { name: 'boby-critical-v1', maxAge: 2592000, maxEntries: 50, strategy: 'cache-first', priority: 10 });
        this.addCache('assets', { name: 'boby-assets-v1', maxAge: 604800, maxEntries: 200, strategy: 'stale-while-revalidate', priority: 7 });
        this.addCache('api', { name: 'boby-api-v1', maxAge: 300, maxEntries: 100, strategy: 'network-first', priority: 3 });
        this.addCache('static', { name: 'boby-static-v1', maxAge: 7776000, maxEntries: 300, strategy: 'cache-first', priority: 5 });
    }
    ...
    public async handleRequest(request: Request): Promise<Response | null> {
        ...
        switch (config.strategy) {
            case 'cache-first': return this.strategyCacheFirst(ctx);
            case 'network-first': return this.strategyNetworkFirst(ctx);
            case 'stale-while-revalidate': return this.strategySWR(ctx);
            default: return null;
        }
    }

    private strategyCacheFirst(ctx: { req: Request, cached: Response | undefined, type: string, key: string, config: CacheConfig }): Promise<Response | null> {
    ...
    private async strategyNetworkFirst(ctx: { req: Request, cached: Response | undefined, type: string, key: string }): Promise<Response | null> {
        try {
    ...
    private strategySWR(ctx: { req: Request, cached: Response | undefined, type: string, key: string, config: CacheConfig }): Promise<Response | null> {
        if (ctx.cached && !this.isEntryExpired(ctx.type, ctx.key, ctx.config.maxAge)) {
            this.updateAccessStats(ctx.type, ctx.key);
            fetch(ctx.req).then(async (res) => {
                if (res.ok) {
    ...
    private async fetchAndStore(req: Request, type: string, key: string, fallback?: Response): Promise<Response | null> {
        try {
    ...
    private async storeInCache(type: string, key: string, res: Response): Promise<void> {
        ...
        entries.set(key, { url: key, response: res.clone(), timestamp: Date.now(), accessCount: 1, size });
    ...
    private async evictGlobalLRU(): Promise<void> {
        ...
        await Promise.all(toEvict.map(async ({ type, key }) => {
            const entries = this.entries.get(type);
            const entry = entries?.get(key);
            if (entry) {
                this.totalCacheSize -= entry.size;
    ...
    private updateAccessStats(type: string, key: string): void {
        const entry = this.entries.get(type)?.get(key);
        if (entry) { entry.accessCount++; entry.timestamp = Date.now(); }
    }

    public getCacheStats() {
        const stats: Record<string, unknown> = { totalSize: this.totalCacheSize, maxTotalSize: this.maxTotalSize };
