...
export class BranchGenerator {
    ...
    generateBranch(branch: Branch): SectionData[] {
        ...
        const levelsDenom = (this.options.type === TreeType.Deciduous ? this.options.branch.levels - 1 : 1);
    ...
    private generateSegmentVertices(
        params: {
            branch: Branch;
            radius: number;
    ...
    private perturbOrientation(orientation: THREE.Euler, radius: number, level: number): void {
        const gnarliness =
            Math.max(1, 1 / Math.sqrt(radius)) *
            (this.options.branch.gnarliness?.[level] ?? 0);
        ...
        qSection.rotateTowards(
            qForce,
            this.options.branch.force.strength / radius,
        );
