...
// تعريف مسارات النسيج للوصول إليها في بيئة المتصفح
const texturePaths = {
...
const loadTexture = (image: string | StaticImageData, srgb: boolean = true): THREE.Texture => {
  ...
  if (typeof image === 'string') {
    // إذا كان اسم النسيج موجودًا في texturePaths، استخدم المسار المحدد
...
export function getBarkTexture(type: BarkType, mapType: 'ao' | 'color' | 'normal' | 'roughness', textureScale: { x: number; y: number }): THREE.Texture | null {
