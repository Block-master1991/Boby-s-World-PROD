...
export type {
    AdaptiveLimits, BehavioralStats,
    PatternAnalysisResult, RateLimitResult, SuspiciousActivity
};
...
export class AdvancedRateLimiter {
    ...
    public async checkRateLimit(
        request: Request,
        identifier: string,
        context: { endpoint: string; deviceInfo?: DeviceInfo | undefined; options?: { customLimit?: number; bypassMode?: boolean } | undefined }
    ): Promise<RateLimitResult> {
        try {
            const { endpoint, deviceInfo, options } = context;
            const ip = getClientIp(request);
            if (this.isLocalhost(ip) || options?.bypassMode) return { allowed: true, action: 'allow' };
    ...
    private async getEnforcementResults(id: string, ep: string, req: Request, limits: AdaptiveLimits) {
        ...
        try {
            ...
            return {
                ...
                pattern: { type: 'rapid_requests', description: 'Fallback active', evidence: null, score: 0 } as PatternAnalysisResult
            };
        }
    }

    private async handleEnforcementAction(ip: string, ep: string, result: RateLimitResult, pattern: PatternAnalysisResult) {
        try {
