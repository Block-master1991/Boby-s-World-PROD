...
export class ConnectionPool {
  ...
  private handleRequestFailure(request: QueuedRequest, error: Error): void {
    ...
    if (request.retryCount < this.config.retryAttempts) {
      ...
      logger.log(
        `[ConnectionPool] Retrying request ${request.id} (attempt ${request.retryCount + 1})`
      );
    } else {
      request.reject(
        new Error(`Request failed after ${this.config.retryAttempts} attempts: ${error.message}`)
      );
    }
  }
