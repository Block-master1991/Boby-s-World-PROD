...
function logValidationCompletion(result: ValidationResult): void {
  logger.log(
    `[AssetValidator] Validation complete: ${result.validAssets.length}/${result.totalChecked} assets valid`
  );
  if (result.missingAssets.length > 0)
    logger.warn(`[AssetValidator] Missing: ${result.missingAssets.length}`);
  if (result.corruptedAssets.length > 0)
    logger.warn(`[AssetValidator] Corrupted: ${result.corruptedAssets.length}`);
}
...
): AssetHealthReport["estimatedGameplayImpact"] {
...
function generateRecommendations(v: ValidationResult): string[] {
  ...
  if (v.missingAssets.length > 0)
    recs.push(`Reload to fetch ${v.missingAssets.length} missing assets`);
  if (v.corruptedAssets.length > 0)
    recs.push(`Found ${v.corruptedAssets.length} corrupted assets - clear cache`);
 ...
 * Public API
 */
export async function canGameStart(): Promise<{
  canStart: boolean;
  reason?: string;
  ...
  if (report.estimatedGameplayImpact === "unplayable")
    return { canStart: false, reason: "Critical assets missing", report };
