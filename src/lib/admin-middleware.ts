...
async function verifyAdminIdentity(publicKey: string, ip: string): Promise<NextResponse | null> {
  ...
  if (publicKey !== ADMIN_WALLET_ADDRESS) {
    ...
    return createAuthErrorResponse({
      message: "Unauthorized. You are not an administrator.",
      code: "ACCESS_DENIED",
      status: 403,
    });
  }
...
async function handleAdminSession(
  ...
  publicKey: string
): Promise<NextResponse | { securityLevel: string }> {
  ...
  if (
    !["biometric", "totp", "mfa"].includes(validation.session.authMethod || "") &&
    !isDev &&
    !isLocal
  ) {
...
export function withAdminAuth<T extends unknown[]>(
  handler: (req: AdminRequest, ...args: T) => Promise<NextResponse>
) {
  return withAuth(async (request: AuthenticatedRequest, ...args: T): Promise<NextResponse> => {
    ...
    if ("status" in sessionRes || sessionRes instanceof NextResponse)
      return sessionRes as NextResponse;
    ...
    logger.log(
      `[AdminMiddleware] Admin access granted to ${publicKey} (Security: ${securityLevel})`
    );
    await auditLogger.logEvent(
      "LOGIN_SUCCESS",
      "Admin access granted",
      { userId: publicKey, role: "ADMIN", securityLevel },
      "info"
    );
 ...
 */
export function withSignedAdminAuth<T extends unknown[]>(
  handler: (req: AdminRequest, ...args: T) => Promise<NextResponse>
) {
  return withAdminAuth(async (request: AdminRequest, ...args: T): Promise<NextResponse> => {
    ...
    if (isNaN(actionTime) || Math.abs(Date.now() - actionTime) > 5 * 60 * 1000) {
      return createAuthErrorResponse({
        message: "Signature expired or invalid.",
    ...
    try {
      ...
      if (
        !verifySignature(constructSignedMessage(timestamp, bodyText), signature, request.user.sub)
      ) {
