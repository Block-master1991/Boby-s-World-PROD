...
export class SecurityLogger {
  ...
  public async logEvent(event: Omit<SecurityEvent, "timestamp">): Promise<void> {
    ...
    logger.warn(
      `[SECURITY_EVENT][${fullEvent.level.toUpperCase()}] ${fullEvent.type}: ${fullEvent.message}`,
      fullEvent.evidence || ""
    );

    // 2. Send Slack alert for critical events
    if (
      fullEvent.level === SecurityEventLevel.CRITICAL ||
      fullEvent.level === SecurityEventLevel.ERROR
    ) {
