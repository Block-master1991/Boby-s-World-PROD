...
export class RecoveryService {
     ...
     */
    static async checkEligibility(publicKey: string): Promise<{ allowed: boolean; reason?: string; status?: number }> {
        if (!redis) {
        ...
        if (attempts && parseInt(attempts) >= RECOVERY_RATE_LIMIT) {
            return { allowed: false, reason: 'Too many recovery attempts. Please try again later.', status: 429 };
        }

        const recoveryKey = `recovery_in_progress:${publicKey}`;
        const recoveryInProgress = await redis.get(recoveryKey);
        if (recoveryInProgress) {
            return { allowed: false, reason: 'Recovery already in progress. Please check your email.', status: 409 };
        }
     ...
     */
    static async verifyUserRecord(publicKey: string, email?: string): Promise<{ exists: boolean; hasPasskeys: boolean; emailMatch: boolean }> {
     ...
     */
    static async initiateRecovery(publicKey: string, email: string, metadata: AuditEventMetadata = {}): Promise<string | null> {
        const recoveryToken = crypto.randomUUID();
        const recoveryCode = Math.random().toString(36).substring(2, 8).toUpperCase();
        
        try {
            // Store state in Redis
            await redis.setex(`recovery_token:${recoveryToken}`, RECOVERY_TOKEN_EXPIRY, JSON.stringify({
                publicKey,
                email,
                recoveryCode,
                timestamp: Date.now()
            ...
            await auditLogger.logEvent('ACCOUNT_RECOVERY_INITIATED', 'Account recovery started', { ...metadata, userId: publicKey, email }, 'warn');
     ...
     */
    static async resetAccount(recoveryToken: string, state: RecoveryState, metadata: AuditEventMetadata = {}): Promise<boolean> {
        ...
        try {
            ...
            await auditLogger.logEvent('ACCOUNT_RECOVERY_VERIFIED', 'Account recovery successful', { ...metadata, userId: publicKey }, 'warn');

            // Send follow-up email
            await emailService.sendCustomEmail(state.email, 'Account Recovery Successful', 
     ...
     */
    static async cancelRecovery(recoveryToken: string, publicKey: string, metadata: AuditEventMetadata = {}): Promise<void> {
        await redis.del(`recovery_token:${recoveryToken}`);
        await redis.del(`recovery_in_progress:${publicKey}`);
        await auditLogger.logEvent('ACCOUNT_RECOVERY_CANCELLED', 'Account recovery cancelled', { ...metadata, userId: publicKey }, 'info');
    }
}
