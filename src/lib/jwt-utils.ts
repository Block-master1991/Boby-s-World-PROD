...
export class JWTManager {
  ...
  private static async validateTokenPayload(
    ...
    if (decoded.type !== expectedType) {
      logger.warn(`[JWTManager] Invalid token type for ${decoded.jti}. Expected '${expectedType}', got '${decoded.type}'.`);
      return false;
    }
    return true;
  }
  ...
  private static async handleVerificationError(
    ...
    logger.error(`[JWTManager] ${expectedType} token verification failed. JTI: ${jti}`, errorMessage);
  ...
  static verifyAccessToken(token: string, userAgent: string, ip: string): Promise<JWTPayload | null> {
  ...
  static verifyRefreshToken(token: string, userAgent: string, ip: string): Promise<JWTPayload | null> {
    ...
    try {
      ...
      if (!decoded || !decoded.jti || !decoded.exp) {
        try {
          ...
          decoded = (jwt.verify(token, this.ACCESS_TOKEN_SECRET) as JWTPayload) || 
      ...
      logger.error('[JWTManager] Error during revocation', error instanceof Error ? error.message : 'Unknown');
      return false;
    }
  }
