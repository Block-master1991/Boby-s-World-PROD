...
export class AwsKmsProvider implements KMSProvider {
  ...
  constructor(config: {
    ...
    logger.log(
      `[AwsKmsProvider] Initialized for region ${config.region} with KeyID ${config.keyId}`
    );
  }
  ...
  securelyClearKey(key: CryptoKey): Promise<void> {
    ...
    logger.log(
      `[AwsKmsProvider] Key usage completed for key type: ${key.algorithm}. Ensure garbage collection removes reference.`
    );
