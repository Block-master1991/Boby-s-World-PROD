...
export class CloudKMSProvider implements KMSProvider {
  ...
  constructor(
    private config: { projectId: string; location: string; keyRing: string; keyId: string }
  ) {}

  async generateKey(algorithm: RsaHashedKeyGenParams | EcKeyGenParams): Promise<CryptoKey> {
    logger.log(
      `[CloudKMS] Will connect to Cloud KMS to create key reference in ${this.config.keyRing}`
    );
