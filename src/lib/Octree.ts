...
class Octree<T> {
  ...
  private insertIntoNode(node: OctreeNode<T>, object: OctreeObject<T>): boolean {
    ...
    if (
      (node.isLeaf() && node.objects.length < this.maxObjectsPerNode) ||
  ...
  private subdivide(node: OctreeNode<T>): void {
    ...
    const subBounds = [
      new THREE.Box3(
        ...
        new THREE.Vector3(min.x + halfSize.x, min.y + halfSize.y, min.z + halfSize.z)
      ),
      new THREE.Box3(
        new THREE.Vector3(min.x + halfSize.x, min.y, min.z),
        new THREE.Vector3(max.x, min.y + halfSize.y, min.z + halfSize.z)
      ),
      new THREE.Box3(
        new THREE.Vector3(min.x, min.y + halfSize.y, min.z),
        new THREE.Vector3(min.x + halfSize.x, max.y, min.z + halfSize.z)
      ),
      new THREE.Box3(
        new THREE.Vector3(min.x, min.y, min.z + halfSize.z),
        new THREE.Vector3(min.x + halfSize.x, min.y + halfSize.y, max.z)
      ),
      new THREE.Box3(
        new THREE.Vector3(min.x + halfSize.x, min.y + halfSize.y, min.z),
        new THREE.Vector3(max.x, max.y, min.z + halfSize.z)
      ),
      new THREE.Box3(
        new THREE.Vector3(min.x + halfSize.x, min.y, min.z + halfSize.z),
        new THREE.Vector3(max.x, min.y + halfSize.y, max.z)
      ),
      new THREE.Box3(
        new THREE.Vector3(min.x, min.y + halfSize.y, min.z + halfSize.z),
        new THREE.Vector3(min.x + halfSize.x, max.y, max.z)
      ),
      new THREE.Box3(
        new THREE.Vector3(min.x + halfSize.x, min.y + halfSize.y, min.z + halfSize.z),
