...
interface UseSolanaAuthProps {
    authState: AuthState; setAuthState: React.Dispatch<React.SetStateAction<AuthState>>; logoutAndRedirect: (path?: string) => Promise<void>;
}
...
export const useSolanaAuth = ({ setAuthState, logoutAndRedirect }: UseSolanaAuthProps) => {
    ...
    const login = useCallback(async (): Promise<boolean> => {
        ...
        try {
            ...
            try { signature = await (wallet?.adapter as any).signMessage(msgBytes, { display: JSON.stringify({ title: 'Boby World', domain: window.location.hostname }) }); } catch { signature = await signMessage(msgBytes); }
            
            const loginRes = await fetch('/api/auth/login', { method: 'POST', headers: { 'Content-Type': 'application/json' }, credentials: 'include', body: JSON.stringify({ publicKey: publicKey.toString(), signature: Buffer.from(signature).toString('hex'), nonce }) });
            const data: LoginResponse = await loginRes.json();
            if (!loginRes.ok) { if (loginRes.status === 403) { await logoutAndRedirect('/'); return false; } throw new Error(data.error || 'Login failed.'); }
            if (data.success && data.publicKey) {
                ...
                toast({ title: 'Login Successful', description: `Wallet: ${data.publicKey.slice(0, 8)}...` });
                return true;
            }
            ...
            toast({ variant: 'destructive', title: 'Login Error', description: msg }); return false;
        } finally { loginInProgressRef.current = false; }
