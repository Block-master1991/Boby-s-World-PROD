...
const useInstanceMutation = (
  ...
  const switchLOD = useCallback(
    (instanceKey: string, newLOD: "high" | "medium" | "low") => {
...
const useVisibilityCulling = (
  ...
  const updateVisibilityBasedOnFrustum = useCallback(
    (camera: THREE.Camera) => {
      ...
      lodMeshesRef.current.forEach(m =>
        Object.values(m).forEach(mesh => mesh && (mesh.instanceMatrix.needsUpdate = true))
      );
      return changes;
    },
...
export const useLODSwitching = () => {
  const { lodModelPathsRef, lodMeshesRef, instancesRef, frustumRef, cameraProjectionMatrixRef } =
    useLODRefs();
  ...
  const { updateFrustum, updateVisibilityBasedOnFrustum } = useVisibilityCulling(
    lodMeshesRef,
    instancesRef,
    frustumRef,
    cameraProjectionMatrixRef
  );

  const createLODMeshes = useCallback(
    (
      modelIdentifier: string,
      models: Record<"high" | "medium" | "low", THREE.Group>,
      maxCount: number
    ) => {
      ...
      const meshes = {
        high: createMesh(models.high, "high"),
        medium: createMesh(models.medium, "medium"),
        low: createMesh(models.low, "low"),
      };
  ...
  const updateLODBasedOnDistance = useCallback(
    (instanceKey: string, distance: number, camera?: THREE.Camera) => {
      const instance = instancesRef.current.get(instanceKey);
      if (!instance) return;
      const pos = new THREE.Vector3().setFromMatrixPosition(instance.transform);
  ...
  return {
    initializeLODModels: useCallback(
      (models: Record<string, LODModelPaths>) => {
    ...
    updateFrustum,
    isInFrustum: (pos: THREE.Vector3, r = 1) =>
      frustumRef.current.intersectsSphere(new THREE.Sphere(pos, r)),
