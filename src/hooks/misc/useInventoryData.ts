...
export const useInventoryData = (
  ...
  const {
    ...
    error: graphqlError,
    execute: refetchInventory,
  } = useUserInventory(sessionPublicKey?.toBase58() || "");
  const { loading: useItemLoading } = useConsumableItem();

  useEffect(() => {
    getStoreItemsActiveWithIcons()
      .then(setStoreItemsData)
      .catch(err => logger.error("Error loading store:", err))
      .finally(() => setStoreItemsLoaded(true));
  }, []);
  ...
  const aggregatedInventory = useMemo(() => {
    return Object.entries(finalCounts)
      ...
      .filter(
        (item): item is { definition: StoreItemDefinition; count: number } => !!item.definition
      );
