...
const createCoin = (
  ...
  Object.assign(coin, {
    collected: false,
    value: COIN_VALUE,
    rotationSpeed: COIN_ROTATION_SPEED,
    userData: {},
    spawnKey,
  });
...
export const useCoinSpawning = (props: SpawningProps) => {
  ...
  const loadCoinsForChunk = useCallback(
    async (cX: number | undefined, cZ: number | undefined) => {
      if (cX === undefined || cZ === undefined) return;
      const k = getChunkKey(cX, cZ);
      if (!sceneRef.current || loadedCoinChunks.current.has(k) || loadingCoinChunks.current.has(k))
        return;
      ...
      try {
        ...
        loadingCoinChunks.current.delete(k);
      }
    },
    [sceneRef, isCoinModelLoadedRef, loadCoinModel, props]
  );

  const unloadCoinsFromChunk = useCallback(
    (cX: number | undefined, cZ: number | undefined) => {
      if (cX === undefined || cZ === undefined) return;
      ...
      props.coinMeshesRef.current = props.coinMeshesRef.current.filter(c => {
        ...
        if (isTargetChunk) {
          ...
          if (props.octreeRef.current) {
            props.octreeRef.current.remove({
              id: `coin_${c.uuid}`,
              bounds: new THREE.Box3().setFromObject(c),
              data: c as unknown as GameObject,
            });
          }
          return false;
        }
