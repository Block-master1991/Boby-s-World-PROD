...
export interface CoinData extends THREE.Mesh {
...
export const useCoinLogic = ({
  ...
  const loadCoinsForChunk = useCallback((chunkX: number, chunkZ: number) => {
    ...
    for (let i = 0; i < numCoinsToGenerate; i++) {
      const coinMesh = new THREE.Mesh(coinGeometry.current, coinMaterial.current) as CoinData; // Cast to CoinData
      // coinMesh.uuid is already a unique string generated by Three.js
      let coinX, coinZ;
      let attempts = 0;
      ...
      if (octreeRef.current) {
        ...
        octreeRef.current.insert({
          id: `coin_${coinMesh.uuid}`, // Use coinMesh.uuid
          bounds: coinBox,
          data: coinMesh
        });
      }
