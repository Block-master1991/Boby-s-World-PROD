...
export const useOctreeManagement = (
  octreeRef: MutableRefObject<Octree<GameObject> | null>
) => {
  const addToOctree = useCallback((object: DynamicLoadableObject) => {
  ...
  const removeFromOctree = useCallback((object: DynamicLoadableObject) => {
    if (octreeRef.current && object.modelInstance) {
