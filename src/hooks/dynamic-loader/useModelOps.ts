...
export const useModelInstanceLoader = ({
  sceneRef, octreeRef, modelPoolRef, gltfLoaderRef, onModelLoaded
}: ModelLoaderOptions) => {
  const { setupAnimations } = useModelAnimations();
  const { addToOctree } = useOctreeManagement(octreeRef);

  return useCallback(async (object: DynamicLoadableObject) => {
    ...
    if (poolEntry?.instances?.length) {
      ...
      } catch (e) { return logger.error(`Load error ${modelName}:`, e); }
    }
    ...
    object.modelInstance.traverse(c => { if ((c as THREE.Mesh).isMesh) { c.castShadow = true; c.receiveShadow = true; } });
...
export const useModelOps = (options: ModelOpsOptions) => {
  ...
  const unloadModel = useCallback((object: DynamicLoadableObject) => {
    ...
    if (!modelPoolRef.current[object.modelPath]) {
      modelPoolRef.current[object.modelPath] = { geometry: null, materials: [], animations: [], instances: [] };
    }
