...
export const useModelAnimations = () => {
  const processClips = useCallback(
    (mixer: THREE.AnimationMixer, clips: THREE.AnimationClip[], object: DynamicLoadableObject) => {
      clips.forEach(clip => {
        ...
        const isLoop =
          (names.IDLE as readonly string[]).includes(clip.name) ||
          clip.name === names.WALK ||
          clip.name === names.GALLOP;
  ...
  const setupAnimations = useCallback(
    (object: DynamicLoadableObject, animations: THREE.AnimationClip[]) => {
