        ...
        const run = async () => {
            try {
                ...
                if (result.allPresent) {
                    setIsCheckOnly(true);
                    setTimeout(() => { if (mounted) onComplete(); }, 2000);
                    return;
                }
                setIsInitialLoading(false);
                const success = await initialAssetPreloader.preloadAllAssets({
                    onProgress: (p) => { if (mounted) handleProgress(p); },
                    maxConcurrentLoads: 3
                });
                if (mounted && success) setTimeout(() => onComplete(), 500);
                else if (mounted) throw new Error('Preload failed');
            } catch (e) { if (mounted) onError(e instanceof Error ? e.message : 'Unknown'); }
        };
        run();
        return () => { mounted = false; initialAssetPreloader.cancelPreload(); };
