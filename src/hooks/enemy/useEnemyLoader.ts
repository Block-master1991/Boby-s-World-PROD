...
export const useEnemyLoader = () => {
  ...
  useEffect(() => {
    ...
    d.setDecoderPath("/libs/draco/gltf/");
    loader.current = new GLTFLoader();
    loader.current.setDRACOLoader(d);
    return () => {
      d.dispose();
      loader.current = null;
    };
  ...
  const loadEnemyModel = useCallback(
    async (t: "carnivore" | "herbivore", f?: string): Promise<R> => {
      const m = t === "carnivore" ? CARN : HERB,
        r = f ?? m[Math.floor(Math.random() * m.length)],
        n = `enemy_${r}`;
      if (cache[n])
        return {
          model: SkeletonUtils.clone(cache[n].model) as THREE.Group,
          animations: cache[n].animations,
        };
      if (loading[n]) {
        const x = await loading[n];
        return { model: SkeletonUtils.clone(x.model) as THREE.Group, animations: x.animations };
      }
      ...
      const p = (async () => {
        try {
          ...
          const x = await load(
            loader.current,
            `/models/Enemies-Animals/${t === "carnivore" ? "Carnivores" : "Herbivores"}/${r}`,
            n
          );
  ...
  const preloadModels = useCallback(async () => {
    if (preloaded) return;
    logger.log("[useEnemyLoader] Preloading...");
    await Promise.all([
      ...CARN.map(n => loadEnemyModel("carnivore", n)),
      ...HERB.map(n => loadEnemyModel("herbivore", n)),
    ]);
    preloaded = true;
    logger.log("[useEnemyLoader] Done.");
  }, [loadEnemyModel]);
  ...
  const disposeModel = useCallback((m: THREE.Object3D) => {
    m.traverse(c => {
      const x = c as THREE.Mesh;
      if (x.isMesh) {
        x.geometry?.dispose();
        const mt = x.material;
        if (Array.isArray(mt)) mt.forEach(z => z.dispose());
        else (mt as THREE.Material)?.dispose();
      }
    });
  }, []);
