...
export const createQueueHandler = (params: QueueHandlerParams) => {
  ...
  const queueSpawns = useCallback((near: Set<string>) => {
    coinMeshesRef.current.forEach(c => {
      const k = c.chunkKey ?? '';
      if (!c.collected && near.has(k) && !enemyMeshesRef.current.some(e => e.targetCoinId === c.uuid) && !pending.has(c.uuid)) {
        queue.push({ coin: c, chunk: k });
      }
    });
  }, [coinMeshesRef, enemyMeshesRef, pending]);

  const process = useCallback(async (spawnFn?: (coin: CoinData, chunk: string) => Promise<void>) => {
