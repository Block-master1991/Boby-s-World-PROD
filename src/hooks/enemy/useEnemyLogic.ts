...
interface Props {
  ...
  loadedCoinChunks: MutableRefObject<Set<string>>;
...
interface Ctx {
  ...
  perf: ReturnType<typeof getDevicePerformanceConfig>;
  frame: number;
...
const getAnimD = (d: number, dt: number, isMob: boolean) =>
  d > 120 ? dt * 0.2 : d > 80 ? dt * 0.5 : d > 40 && isMob ? dt * 0.8 : dt;
...
const animE = (e: EnemyData, d: number, inF: boolean, ctx: Ctx) => {
  ...
  // Update dither fade-in
  const data = e as EnemyData & { _fadeDone?: boolean; _ditherVal?: number };
  if (!data._fadeDone && e.lod.visible) {
    const current = data._ditherVal || 0;
    const next = Math.min(1.0, current + ctx.delta * 2.0); // 0.5s fade
    ...
    data._ditherVal = next;
    if (next >= 1.0) data._fadeDone = true;
  }
};

const dispE = (e: EnemyData) => {
...
const initializeEnemyHooks = (
  ...
  const { manageChunks, initializeEnemies, resetEnemies, forceLoadAreaEnemies, spawn } =
    useEnemySpawning({
      sceneRef,
      octreeRef,
  ...
  const { updateEnemyMovement } = useEnemyMovement({
    dogModelRef,
    octreeRef,
    sceneRef,
    isPausedRef,
  ...
  return React.useCallback(
    (e: EnemyData, ctx: Ctx) => {
      ...
      if (!protectedCoin && !e.isDying) {
        ...
        const c =
          e.enemyType === "carnivore"
            ? ENEMY_ANIMATION_NAMES.CARNIVORE
            : ENEMY_ANIMATION_NAMES.HERBIVORE;
        const action = e.actions[c.DEATH];
        if (action) {
...
const createUpdateEnemiesCallback = (params: UpdateEnemiesParams) => {
  ...
  return React.useCallback(
    (delta: number) => {
      if (isPausedRef.current || !dogModelRef.current || !sceneRef.current || !cameraRef.current)
        return;
      manageChunks();
...
export const useEnemyLogic = (props: Props) => {
  const enemies = React.useRef<EnemyData[]>([]),
    frameRef = React.useRef(0);
  ...
  const updateOne = createUpdateOneCallback(
    props.coinMeshesRef,
    updateEnemyMovement,
    checkCollisions,
    updateOctree
  );
  const filterDead = createFilterDeadCallback(
    enemies,
    updateDeathState,
    removeFromOctree,
    props.sceneRef
  ...
  return {
    ...
    forceLoadAreaEnemies,
    enemyMeshesRef: enemies,
    getPreloadableEnemies: getPreloadableModels,
  };
};
