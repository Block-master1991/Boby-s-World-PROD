...
export const createMovementUpdater = (
  params: MovementUpdaterParams & { octreeRef: React.MutableRefObject<Octree<GameObject> | null> }
) => {
  ...
  const applyMovement = (
    ...
    if (direction.lengthSq() > movementThreshold) {
      direction.normalize();
      const speed =
        currentAnimation === "Gallop" ? ENEMY_SPEED * ENEMY_GALLOP_SPEED_MULTIPLIER : ENEMY_SPEED;
  ...
  const updateMovement = useCallback(
    (e: EnemyData, dt: number, dist: number) => {
