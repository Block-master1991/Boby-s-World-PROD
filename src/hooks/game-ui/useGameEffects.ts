...
const useBuffTimer = (duration: number, name: string, endMsg: string) => {
    ...
    const activate = useCallback((amt: number) => {
        ...
        return () => { stop(); toast({ title: `${name} Cancelled`, variant: "destructive" }); };
...
export const useGameEffects = () => {
    ...
    return {
        isSpeedBoostActive: sb.isActive, speedBoostTimeLeft: sb.timeLeft, activateSpeedBoost: sb.activate,
        isShieldActive: sh.isActive, shieldTimeLeft: sh.timeLeft, activateGuardianShield: sh.activate,
        isCoinMagnetActive: cm.isActive, coinMagnetTimeLeft: cm.timeLeft, activateCoinMagnet: cm.activate,
