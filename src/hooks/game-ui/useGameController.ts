...
export const useGameController = ({ onSheetsStateChange }: UseGameControllerProps) => {
  ...
  const inventory = useGameInventory({
    ...
    activateSpeedBoost: effects.activateSpeedBoost,
    activateGuardianShield: effects.activateGuardianShield,
    activateCoinMagnet: effects.activateCoinMagnet,
  });
  ...
  const toggleSheet = (key: keyof typeof sheets, val: boolean) =>
    setSheets(p => ({ ...p, [key]: val }));
  const isPaused = Object.values(sheets).some(Boolean) || isWalletMismatch;

  // Input
  const joystick = useJoystickControls({
    isMobile,
    isGameEffectivelyPaused: isPaused,
    isAuthenticated,
    isWalletConnectedAndMatching,
  });

  useEffect(() => {
    if (isAuthenticated && authUser?.publicKey) fetchPlayerData();
  }, [isAuthenticated, authUser, fetchPlayerData]);
  useEffect(() => {
  ...
  const overlayProps = useMemo(
    () => ({
      sessionCollectedUSDT: economy.displayedPlayerGameUSDT,
      remainingCoinsOnMap: economy.remainingCoinsOnMap,
      ...
      speedBoostTimeLeft: effects.speedBoostTimeLeft,
      isShieldActive: effects.isShieldActive,
      shieldTimeLeft: effects.shieldTimeLeft,
      isCoinMagnetActive: effects.isCoinMagnetActive,
      coinMagnetTimeLeft: effects.coinMagnetTimeLeft,
      speedyPawsTreatCount: inventory.displayedSpeedyPawsTreatCount,
      guardianShieldCount: inventory.displayedGuardianShieldCount,
      coinMagnetTreatCount: inventory.displayedCoinMagnetTreatCount,
      onUseConsumableItem: inventory.handleUseConsumableItem,
      isGameEffectivelyPaused: isPaused,
      isWalletMismatch,
      isMobile,
      dynamicJoystickState: joystick.dynamicJoystickState,
      JOYSTICK_BASE_SIZE: joystick.JOYSTICK_BASE_SIZE,
      JOYSTICK_KNOB_SIZE: joystick.JOYSTICK_KNOB_SIZE,
      ...(inventory.protectionBottleDef
        ? { protectionBottleDef: inventory.protectionBottleDef }
        : {}),
