...
interface EconomyState {
  ...
  unbatchedCoin: number;
  unbatchedPenalty: number;
...
function useCoinBatchProcessor(
  apiFetch: ApiFetch,
  updateBalanceLocally: (usd: number, syncId?: string) => void,
  ...
  return useCallback(
    async (amounts: number[]) => {
      ...
      setEconomyState((p: EconomyState) => ({
        ...
        optimisticUpdates: [
          ...p.optimisticUpdates,
          { id, type: "coin", amount: total, timestamp: Date.now(), status: "pending" },
        ],
      }));

      logger.log(`[Economy] Starting coin sync: ${total} USDT (Batch: ${id})`);
      try {
        const res = await apiFetch("/api/game/addCoin", {
          method: "POST",
          body: JSON.stringify({ amount: total, batchId: id }),
        });
        if (res.ok) {
          const data = await res.json();
          logger.log(
            `[Economy] Coin sync success. New Balance: ${data.newBalance} USDT (Batch: ${id})`
          );
        ...
        logger.error(
          `[Economy] Coin sync failed, rolling back: ${total} USDT (Batch: ${id})`,
          e as Error
        );
        setEconomyState((p: EconomyState) => ({
          ...p,
          unbatchedCoin: p.unbatchedCoin + total,
          optimisticUpdates: p.optimisticUpdates.filter(u => u.id !== id),
        }));
      }
    },
...
function usePenaltyBatchProcessor(
  apiFetch: ApiFetch,
  updateBalanceLocally: (usd: number, syncId?: string) => void,
  setEconomyState: SetEconomyState
) {
  return useCallback(
    async (items: PenaltyQueueItem[]) => {
      ...
      setEconomyState((p: EconomyState) => ({
        ...
        optimisticUpdates: [
          ...p.optimisticUpdates,
          { id: batchId, type: "penalty", amount: total, timestamp: Date.now(), status: "pending" },
        ],
      }));

      logger.log(`[Economy] Starting penalty sync: ${total} USDT (Batch: ${batchId})`);
      try {
        const res = await apiFetch("/api/game/applyPenalty", {
          method: "POST",
          body: JSON.stringify({ amount: total, batchId: batchId }),
        });
        if (res.ok) {
          const data = await res.json();
          logger.log(
            `[Economy] Penalty sync success. New Balance: ${data.newBalance} USDT (Batch: ${batchId})`
          );
        ...
        logger.error(
          `[Economy] Penalty sync failed, rolling back: ${total} USDT (Batch: ${batchId})`,
          e as Error
        );
        setEconomyState((p: EconomyState) => ({
          ...p,
          unbatchedPenalty: p.unbatchedPenalty + total,
          optimisticUpdates: p.optimisticUpdates.filter(u => u.id !== batchId),
        }));
      }
    },
    [apiFetch, updateBalanceLocally, setEconomyState]
...
function calculateDisplayedBalance(
  playerGameUSDT: number,
  state: EconomyState,
  lastSyncId: string
) {
...
function useWithdrawalHandler({
  ...
  return useCallback(async () => {
    if (
      ...
      !isWalletConnectedAndMatching ||
      !authUserPublicKey ||
      displayedBalance < MIN_WITHDRAWAL_USDT
    ) {
      toast({
        title: "Unavailable",
        description: `Need ${MIN_WITHDRAWAL_USDT}.`,
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      const res = await apiFetch("/api/game/withdrawUSDT", {
        method: "POST",
      ...
      if (res.ok) {
        ...
        setEconomyState((p: EconomyState) => ({
          ...p,
          optimisticUpdates: p.optimisticUpdates.map(u =>
            u.id === id ? { ...u, status: "failed" } : u
          ),
        }));
      ...
      setEconomyState((p: EconomyState) => ({ ...p, isWithdrawing: false }));
    }
  }, [
    isAuthenticated,
    isWalletConnectedAndMatching,
...
export const useGameEconomy = ({
  ...
  const displayedPlayerGameUSDT = useMemo(
    () => calculateDisplayedBalance(playerGameUSDT, economyState, lastSyncId),
    [playerGameUSDT, economyState, lastSyncId]
  );

  const processCoinBatch = useCoinBatchProcessor(
    apiFetch as ApiFetch,
    updateBalanceLocally,
    setEconomyState
  );
  const processPenaltyBatch = usePenaltyBatchProcessor(
    apiFetch as ApiFetch,
    updateBalanceLocally,
    setEconomyState
  );

  const { addUpdate: batchAddCoin } = useBatchedUpdates<number>(
    processCoinBatch,
    3000,
    "offline_coin_queue_v1"
  );
  const { addUpdate: batchApplyPenalty } = useBatchedUpdates<PenaltyQueueItem>(
    processPenaltyBatch,
    3000,
    "offline_penalty_queue_v1"
  );
  ...
  const handleEnemyCollisionPenalty = useCallback(() => {
    if (!isAuthenticated || !isWalletConnectedAndMatching || !authUserPublicKey) {
      toast({ title: "Blocked", description: "Auth required.", variant: "destructive" });
      return;
    }
    setEconomyState((p: EconomyState) => ({
      ...p,
  ...
  const handleWithdrawUSDT = useWithdrawalHandler({
    ...
    isWalletConnectedAndMatching,
    authUserPublicKey,
    displayedBalance: displayedPlayerGameUSDT,
  ...
  return {
    ...
    handleWithdrawUSDT,
    handleRemainingCoinsUpdate: (v: number) =>
      setEconomyState((p: EconomyState) => ({ ...p, remainingCoins: v })),
    MIN_WITHDRAWAL_USDT,
