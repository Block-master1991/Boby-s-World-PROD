...
export const useGameData = () => {
    ...
    const fetchPlayerData = useCallback(async (loadUi = false) => {
        if (!isAuthenticated || !authUser?.publicKey) { setLoading(p=>({...p, fetch:false})); return; }
        if (loadUi) setLoading({ fetch: true, store: true, inv: true, wallet: true }); else setLoading(p=>({...p, fetch: true}));

        try {
            const res = await fetchData(authUser.publicKey);
    ...
    }, [isAuthenticated, authUser, toast, fetchData]);
