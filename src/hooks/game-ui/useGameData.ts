...
export const useGameData = (props: { sessionPublicKey?: string | undefined }) => {
    ...
    const fetchPlayerData = useCallback(async (loadUi = false) => {
        ...
        if (!key) { setLoading(p=>({...p, fetch:false})); return; }
        ...
        try {
            ...
            if (res?.success && res.playerData) {
                ...
                setState(prev => {
                    ...
                    if (isStale) {
                        logger.log(`[DataSync] REJECTED stale balance from server. ServerBatch: ${incomingBatchId}, LocalSyncId: ${prev.lastSyncId}`);
                    } else {
                        logger.log(`[DataSync] ACCEPTED balance from server: ${res.playerData!.coins} USDT. ServerBatch: ${incomingBatchId}`);
                    }
    ...
    return {
        ...
        updateBalanceLocally, lastSyncId: state.lastSyncId,
