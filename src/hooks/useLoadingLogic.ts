...
export const useLoadingLogic = (p: UseLoadingLogicProps) => {
    const preloadEntities = useCallback(async (dogPos: THREE.Vector3) => {
        const { chunkX, chunkZ } = getChunkCoordinates(dogPos.x, dogPos.z);
        p.currentDogChunkRef.current = { chunkX, chunkZ };
        await Promise.all([p.forceLoadAreaCoins(chunkX, chunkZ), p.forceLoadAreaEnemies(chunkX, chunkZ)]);
        p.setupInitialCameraPosition();
    }, [p.currentDogChunkRef, p.forceLoadAreaCoins, p.forceLoadAreaEnemies, p.setupInitialCameraPosition]);

    const initializeWorldEnvironment = useCallback(async (dogPos: THREE.Vector3) => {
    ...
    const loadAllGameAssets = useCallback(async () => {
        p.onLoadStart();
        try {
            const assetLoader = new PriorityAssetLoader({ rendererRef: p.rendererRef, cameraRef: p.cameraRef, initializeDog: async () => { await p.initializeDog(); }, initializeCoins: async () => { await p.initializeCoins(); }, initializeEnemies: async () => { await p.initializeEnemies(); } });
            await assetLoader.loadAssetsByPriority((d: LPData) => p.onLoadProgress(d.progress, d.phase)); p.onLoadProgress(85, 'world');
        } catch (error) { logger.error("[useLoadingLogic] Asset load error:", error); p.onLoadComplete(false); throw error; }
