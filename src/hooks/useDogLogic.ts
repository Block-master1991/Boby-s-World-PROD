...
export const useDogLogic = (props: DogLogicProps) => {
    const { 
        sceneRef, keysPressedRef, joystickInputRef, isPausedRef, 
        isSpeedBoostActiveRef, isShieldActiveRef, isJoystickInteractionActiveRef, octreeRef 
    } = props;
    
    const { dogModelRef, gltfLoaderRef, setupModel } = useDogAssets(sceneRef);
    const { animationMixerRef, initAnimations, updateAnimationState, stopAnimations } = useDogAnimations();
    const { dogSpeedRef, isRunningRef, applyMovement } = useDogMovement(
        keysPressedRef, joystickInputRef, isJoystickInteractionActiveRef, isSpeedBoostActiveRef
    );
    ...
    const { initializeDog } = useDogInitialization({
        sceneRef, dogModelRef, gltfLoaderRef, lastDogTransformRef, setupModel, initAnimations
    });
    ...
    return { 
        dogModelRef, lastDogTransformRef, initializeDog, updateDog, resetDogState, 
        dogSpeed: dogSpeedRef.current, isRunning: isRunningRef.current 
    };
};
