...
import {
    ...
    NORMAL_DOG_SPEED, SPRINT_DOG_SPEED,
...
const getSprintState = (active: boolean, input: { x: number, y: number } | null, keys: Record<string, boolean>) => {
    if (active && input) {
...
const applyJoystick = (dog: THREE.Group, data: { jX: number, jY: number, speed: number }, delta: number, jRot: number) => {
    let rot = false;
...
const applyKbd = (dog: THREE.Group, delta: number, opts: { speed: number, kRot: number, keys: Record<string, boolean> }) => {
    let rot = false;
    const { speed, kRot, keys } = opts;
    if (keys['KeyA'] || keys['ArrowLeft']) { dog.rotation.y += kRot * delta; rot = true; }
    if (keys['KeyD'] || keys['ArrowRight']) { dog.rotation.y -= kRot * delta; rot = true; }
    const forward = new THREE.Vector3();
    dog.getWorldDirection(forward);
    let mov = false;
    if (keys['KeyW'] || keys['ArrowUp']) { dog.position.addScaledVector(forward, speed * delta); mov = true; }
    if (keys['KeyS'] || keys['ArrowDown']) { dog.position.addScaledVector(forward, -speed * delta); mov = true; }
    return { rot, mov };
};
...
export const useDogMovement = (
    ...
    const getMovementData = useCallback(() => {
        ...
        const active = isJoystickInteractionActiveRef.current && !!input && (input.x !== 0 || input.y !== 0);
    ...
    const applyMovement = useCallback((dog: THREE.Group, delta: number) => {
        ...
        const res = active ? applyJoystick(dog, { jX, jY, speed }, delta, jRot) : applyKbd(dog, delta, { speed, kRot, keys: keysPressedRef.current });
        dog.position.y = 0;
