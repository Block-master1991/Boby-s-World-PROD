...
export const useDogInitialization = (props: InitializationProps) => {
    const { sceneRef, dogModelRef, gltfLoaderRef, lastDogTransformRef, setupModel, initAnimations } = props;

    const handleFallback = useCallback((scene: THREE.Scene) => {
    ...
    }, [sceneRef, gltfLoaderRef, setupModel, initAnimations, dogModelRef, lastDogTransformRef, handleFallback]);
