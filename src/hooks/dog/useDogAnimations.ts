...
const setupClips = (mixer: THREE.AnimationMixer, clips: THREE.AnimationClip[]) => {
    ...
    const loopNames = [ANIMATION_NAMES.IDLE, ANIMATION_NAMES.WALK, ANIMATION_NAMES.RUN, ANIMATION_NAMES.SPRINT_JUMP] as readonly string[];
    clips.forEach((clip) => {
...
export const useDogAnimations = () => {
    ...
    const initAnimations = useCallback((gltf: { animations: THREE.AnimationClip[] }, model: THREE.Group) => {
        if (!gltf.animations?.length) return;
        const mixer = new THREE.AnimationMixer(model);
        animationMixerRef.current = mixer;
        animationActionsRef.current = setupClips(mixer, gltf.animations);
        const iName = animationActionsRef.current[ANIMATION_NAMES.IDLE] ? ANIMATION_NAMES.IDLE : gltf.animations[0]?.name;
