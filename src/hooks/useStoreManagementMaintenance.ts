...
export const useStoreManagementMaintenance = (state: ReturnType<typeof useStoreManagementState>, loadItems: () => Promise<void>) => {
    ...
    const handleImageUpload = async (file: File) => {
        ...
        try {
            ...
            const res = await apiFetch('/api/admin/upload-image', { method: 'POST', headers, body: uploadFormData });
    ...
    const runMaintenanceTask = async (task: 'migrate-inventory' | 'migrate-images') => {
        ...
        try {
            ...
            const data = await res.json();
            toast({ title: data.success ? 'Success' : 'Failed', description: data.message || data.error });
