...
export const useBobyPriceLogic = () => {
    ...
    useEffect(() => {
        ...
        if (graphqlError || subscriptionError) {
            setErrorInfo({ message: graphqlError || subscriptionError || 'Failed to fetch price', details: 'Failed to fetch price via GraphQL' });
            setPrice(null);
    ...
    const fetchPrice = useCallback(async (isInitialLoad = false) => {
        ...
        setErrorInfo(null);
        try { await refreshPrice(); } catch (e: unknown) {
            setErrorInfo({ message: (e instanceof Error) ? e.message : 'Failed to refresh price', details: 'GraphQL refresh failed' });
            setPrice(null); setIsLoading(false);
        }
    }, [refreshPrice]);
