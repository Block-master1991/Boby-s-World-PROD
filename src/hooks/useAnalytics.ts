...
export const useAnalytics = () => {
    ...
    const trackPerformance = useCallback((snapshot: { fps: number; memoryUsage: number; drawCalls: number }) => {
    ...
    const trackError = useCallback((error: Error, context?: Record<string, unknown>) => {
        trackEvent({
            type: 'error',
            data: { message: error.message, stack: error.stack, name: error.name, context: context ?? {} }
        });
    }, [trackEvent]);
    ...
    const trackGameEvent = useCallback((event: string, data?: Record<string, unknown>) => {
