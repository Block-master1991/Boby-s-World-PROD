...
import type { ActivityUpdate, AddCoinsResponse, FetchPlayerDataResponse, UserInventory, UserStats } from './types';
...
export const useUserData = (userId: string) => useBaseGraphQL(GAME_QUERIES.GET_USER_GAME_DATA, { variables: { userId }, skip: !userId });

export const useUserInventory = (userId: string) => useBaseGraphQL<{ userInventory: UserInventory }>(`
...
export const useUserStats = (requiredRole: 'admin' | 'user' = 'user') => {
    ...
    const [state, setState] = useState<{ data: { userStats: UserStats } | null, loading: boolean, error: string | null }>({ data: null, loading: false, error: null });
    ...
    const execute = useCallback(async () => {
        ...
        try {
            const res = await apiFetch('/api/graphql', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ query: QUERIES.USER_STATS }) });
    ...
    useEffect(() => { if (isAuthenticated && isAuthorized) execute(); }, [execute, isAuthenticated, isAuthorized]);
    return { ...state, execute };
};

export const useFetchPlayerData = () => {
    const { apiFetch } = useApiFetch();
    const [state, setState] = useState<{ data: { fetchPlayerData: FetchPlayerDataResponse } | null, loading: boolean, error: string | null }>({ data: null, loading: false, error: null });
...
export const useAddCoins = () => {
    ...
    const addCoins = useCallback(async (userId: string, amount: number): Promise<AddCoinsResponse> => {
        setState({ loading: true, error: null });
        try {
            const res = await apiFetch('/api/graphql', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ query: QUERIES.ADD_COINS, variables: { userId, amount } }) });
            if (!res.ok) throw new Error(`HTTP ${res.status}`);
            const json = await res.json();
            if (json.errors) throw new Error(json.errors[0].message);
...
export const useUserActivityUpdates = (requiredRole: 'admin' | 'user' = 'user') => {
    const { apiFetch } = useApiFetch();
    const { isAuthenticated, user } = useAuth();
    const [state, setState] = useState<{ data: ActivityUpdate | null, error: string | null }>({ data: null, error: null });
    ...
    useEffect(() => {
        ...
        const poll = async () => {
            try {
                const res = await apiFetch('/api/graphql', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ query: QUERIES.ACTIVITY_UPDATES }) });
                if (active && res.ok) {
                    const json = await res.json();
                    if (json.data?.userActivityUpdates) setState({ data: json.data.userActivityUpdates, error: null });
                }
            } catch (err) {
                if (active) {
                    const msg = err instanceof Error ? err.message : 'Activity subscription error';
                    if (!msg.includes('NetworkError')) { logger.error('Error:', msg); setState(s => ({ ...s, error: msg })); }
                }
            }
        };
