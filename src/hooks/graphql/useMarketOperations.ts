...
export const useMarketData = () => {
  ...
  const [state, setState] = useState<{
    data: { marketData: MarketData } | null;
    loading: boolean;
    error: string | null;
  }>({
    data: globalMarketDataCache,
    loading: false,
    error: null,
  });
  ...
  const execute = useCallback(
    async (force = false) => {
      ...
      if (
        ...
        now - lastMarketDataFetchTime < MARKET_DATA_CACHE_TTL
      ) {
      ...
      try {
        const res = await apiFetch("/api/graphql", {
          ...
          body: JSON.stringify({
            query: `query GetMarketData { marketData { bobyPrice volume24h priceChange24h lastUpdated } }`,
          }),
        });
...
export const useBobyPriceUpdates = () => {
  const { apiFetch } = useApiFetch();
  const [state, setState] = useState<{ data: PriceUpdate | null; error: string | null }>({
    data: null,
    error: null,
  });
    ...
    const poll = async () => {
      try {
        const res = await apiFetch("/api/graphql", {
          ...
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            query: `subscription BobyPriceUpdates { bobyPriceUpdates { price changePercent timestamp } }`,
          }),
        });
        if (active && res.ok) {
          const json = await res.json();
          if (json.data?.bobyPriceUpdates)
            setState(s => ({ ...s, data: json.data.bobyPriceUpdates, error: null }));
        }
      } catch (err) {
...
export const useWithdrawUSDT = () => {
  ...
  const withdrawUSDT = useCallback(
    async (userId: string, amount: number): Promise<WithdrawResponse> => {
      setLoading(true);
      setError(null);
      try {
        const res = await apiFetch("/api/graphql", {
        ...
        const msg = err instanceof Error ? err.message : "Unknown error";
        setError(msg);
        logger.error("[useWithdrawUSDT] Error:", msg);
