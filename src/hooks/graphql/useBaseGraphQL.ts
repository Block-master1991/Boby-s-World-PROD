...
export const useBaseGraphQL = <T = unknown>(
  ...
  const cacheKey = useMemo(
    () => `${query.trim()}-${JSON.stringify(options?.variables || {})}`,
    ...
    async (variables?: unknown) => {
      ...
      const req = executeQuery<T>(query, finalVars, cacheKey, cacheTTL)
        ...
        .catch(err => {
          ...
          setError(msg);
          logger.error("[useGraphQL] Query failed:", msg);
          throw err;
        })
        .finally(() => {
          setLoading(false);
...
export const useBaseMutation = <T = unknown>(mutation: string) => {
  ...
  const execute = useCallback(
    async (variables?: unknown): Promise<T | null> => {
      setLoading(true);
      setError(null);
      try {
        const client = getGraphQLClient();
        ...
        const msg = err instanceof Error ? err.message : "Unknown error";
        setError(msg);
        logger.error("[useGraphQLMutation] Mutation failed:", msg);
        return null;
      } finally {
        setLoading(false);
      }
