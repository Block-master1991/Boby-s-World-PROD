...
export const useTouchHandling = ({
  ...
  isPausedRef: iP,
  isJoystickInteractionActiveRef: iJI,
  onCanvasTouchStartProp: oTS,
  onCanvasTouchMoveProp: oTM,
  onCanvasTouchEndProp: oTE,
  initialTouchPointRef: iT,
  ...
  const handleStart = useCallback(
    (e: TouchEvent) => {
      if (e.touches.length === 1 && !iP.current && sPK) {
        ...
        if (t) {
          iJI.current = true;
          iT.current = { x: t.clientX, y: t.clientY, id: t.identifier };
