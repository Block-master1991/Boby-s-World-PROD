...
import { useBobyPrice } from "@/hooks/misc/useBobyPrice";
...
const useQuantityControls = (storeItems: StoreItemDefinition[]) => {
    ...
    if (storeItems.length > 0) {
      setQuantities(prev => {
        ...
        storeItems.forEach(item => {
          if (updated[item.id] === undefined) updated[item.id] = 1;
        });
        return updated;
      });
    }
  ...
  const handleIncrement = useCallback(
    (id: string) => setQuantities(p => ({ ...p, [id]: (p[id] || 1) + 1 })),
    []
  );
  const handleDecrement = useCallback(
    (id: string) => setQuantities(p => ({ ...p, [id]: Math.max((p[id] || 1) - 1, 1) })),
    []
  );
...
export const useStoreState = (storeItems: StoreItemDefinition[], itemsLoading: boolean) => {
  ...
  useEffect(() => {
    if (!itemsLoading) setStoreItemsLoaded(true);
  }, [itemsLoading]);
