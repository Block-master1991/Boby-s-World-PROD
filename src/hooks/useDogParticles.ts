...
const useParticleInitialization = (
  sceneRef: MutableRefObject<THREE.Scene | null>,
  ...
  useEffect(() => {
    ...
    particlesGeometryRef.current.setAttribute(
      "position",
      new THREE.BufferAttribute(state.particlePositions.current, 3)
    );
    particlesGeometryRef.current.setAttribute(
      "opacity",
      new THREE.BufferAttribute(state.particleOpacities.current, 1)
    );
...
export const useDogParticles = ({
  sceneRef,
  dogMeshRef,
  dogSpeed,
  isRunning,
}: UseDogParticlesProps) => {
