  ...
  Mutation: {
    useConsumableItem: async (_: unknown, { itemId, quantity }: { itemId: string, quantity: number }, context: GraphQLContext) => {
      try {
        ...
        return { success: false, message: 'Failed to use item', error: error instanceof Error ? error.message : 'Unknown error' };
      }
    },
    consumeProtectionBottle: async (_: unknown, __: unknown, context: GraphQLContext) => {
      try {
        ...
        return { success: false, message: 'Failed to consume bottle', error: error instanceof Error ? error.message : 'Unknown error' };
      }
    },
  },
  Subscription: {
