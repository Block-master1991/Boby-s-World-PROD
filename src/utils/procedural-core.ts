...
export class ProceduralCore {
  ...
  private static generateCoins(
    rng: RNG,
    chunk: { x: number; z: number; startX: number; startZ: number }
  ): WorldEntity[] {
    const entities: WorldEntity[] = [];
    const isInsideCoinZone =
      Math.abs(chunk.x) <= this.COIN_ZONE_LIMIT && Math.abs(chunk.z) <= this.COIN_ZONE_LIMIT;
  ...
  public static generateGameplayEntities(chunkX: number, chunkZ: number): WorldEntity[] {
    ...
    return [
      ...this.generateCoins(rng, {
        x: chunkX,
        z: chunkZ,
        startX: chunkWorldStartX,
        startZ: chunkWorldStartZ,
      }),
