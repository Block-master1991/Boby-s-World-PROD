...
const getTokenAddresses = async (adapterPublicKey: PublicKey) => {
    ...
    const fromAddress = await Token.getAssociatedTokenAddress(
        ASSOCIATED_TOKEN_PROGRAM_ID, TOKEN_PROGRAM_ID, bobyMintPublicKey, adapterPublicKey
    );
    const toAddress = await Token.getAssociatedTokenAddress(
        ASSOCIATED_TOKEN_PROGRAM_ID, TOKEN_PROGRAM_ID, bobyMintPublicKey, treasuryPublicKey
    );
...
export const buildBobyPurchaseTransaction = async (
    ...
    if (!(await connection.getAccountInfo(addr.toAddress).catch(() => null))) {
        tx.add(Token.createAssociatedTokenAccountInstruction(
            ASSOCIATED_TOKEN_PROGRAM_ID, TOKEN_PROGRAM_ID, addr.bobyMintPublicKey, 
            addr.toAddress, addr.treasuryPublicKey, adapterPublicKey
        ));
    }

    tx.add(Token.createTransferInstruction(
        TOKEN_PROGRAM_ID, addr.fromAddress, addr.toAddress, 
        adapterPublicKey, [], Math.round(amount * (10 ** BOBY_TOKEN_DECIMALS))
    ));

    const { blockhash, lastValidBlockHeight } = await connection.getLatestBlockhash('finalized');
    Object.assign(tx, { recentBlockhash: blockhash, lastValidBlockHeight, feePayer: adapterPublicKey });
