...
import type {
    ChunkData, ChunkWorkerMessage
} from './worker-utils';
import {
    cleanupCache,
    OccupancyGrid, updatePerformanceMetrics
} from './worker-utils';
...
function processGrass(params: { rng: RNG, startX: number, startZ: number, opts: GrassOptions, env: GenOptions, out: InstanceData }) {
...
function generateGrassData(opts: GrassOptions, env: GenOptions) {
    const out = { positions: [] as number[], scales: [] as number[], quaternions: [] as number[], colors: [] as number[] };
...
function processRock(params: { rng: RNG, startX: number, startZ: number, opts: RocksOptions, env: GenOptions, out: InstanceData }) {
    const { rng, startX, startZ, opts, env, out } = params;
    const localX = rng.random(0, CHUNK_SIZE);
    const localZ = rng.random(0, CHUNK_SIZE);
    const worldX = startX + localX;
...
function processTree(params: { rng: RNG, startX: number, startZ: number, opts: TreesOptions, env: GenOptions, out: InstanceData }) {
    const { rng, startX, startZ, opts, env, out } = params;
    const localX = rng.random(0, CHUNK_SIZE);
    const localZ = rng.random(0, CHUNK_SIZE);
    const worldX = startX + localX;
...
function processFlower(params: { rng: RNG, startX: number, startZ: number, opts: FlowerOptions, env: GenOptions, out: InstanceData }) {
    const { rng, startX, startZ, opts, env, out } = params;
    const localX = rng.random(0, CHUNK_SIZE);
    const localZ = rng.random(0, CHUNK_SIZE);
    const worldX = startX + localX;
    ...
    if (worldX < env.worldMin || worldX > env.worldMax || worldZ < env.worldMin || worldZ > env.worldMax) return;
    if ((0.5 + 0.5 * getCachedNoise(worldX, worldZ, opts.scale)) > opts.patchiness - 0.05 || env.grid.isOccupied(localX, localZ, 0.3)) return;
...
function generateGameplayData(chunkX: number, chunkZ: number) {
    ...
    ProceduralCore.generateGameplayEntities(chunkX, chunkZ).forEach(entity => {
        ...
        else if (entity.type === 'ENEMY') enemySpawns.push({ position: [entity.x, 0, entity.z], coinIndex: coinSpawns.length - 1 });
    });
